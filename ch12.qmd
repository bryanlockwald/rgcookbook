...
# Using Colors in Plots {#sec-chapter-colors}
...
## Setting the Colors of Objects {#sec-recipe-colors-setting}
...
In the call to the geom, set the values of `colour` or `fill` (Figure @fig-colors-setting):


```{r}
#| label: fig-colors-setting
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Setting `fill` and `colour` (left); Setting `colour` for points (right)"
#| message: false
#| fig-width: 4
#| fig-height: 3.5
...
For more information about point shapes, see Recipe @sec-recipe-line-graph-point-appearance.

See Recipe @sec-recipe-colors-palette-discrete-manual for more on specifying colors.


## Representing Variables with Colors {#sec-recipe-colors-mapping}
...
In the call to the geom, inside of `aes()`, set the value of `colour` or `fill` to the name of one of the columns in the data (Figure @fig-colors-mapping):
...
```{r}
#| label: fig-colors-mapping
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| message: false
#| fig-cap: "Mapping a variable to `fill` (left); Mapping a variable to `colour` for points (right)"
#| fig-width: 4
#| fig-height: 3.5
library(gcookbook)  # Load gcookbook for the cabbage_exp data set
...
In the `mtcars` example, `cyl` is numeric, so it is treated as a continuous variable. Because of this, even though the actual values of `cyl` include only 4, 6, and 8, the legend has entries for the intermediate values 5 and 7. To make ggplot treat `cyl` as a categorical variable, you can convert it to a factor in the call to `ggplot()` (Figure @fig-colors-mapping-factor, left), or you can modify the data so that the column is a character vector or factor (Figure @fig-colors-mapping-factor, right):


```{r}
#| label: fig-colors-mapping-factor
#| fig-show: "hold"
#| fig-subcap: ["", "", ""]
#| layout-ncol: 3
#| fig-cap: "Converting `cyl` to a factor, within the call to ggplot (left); By modifying the dataframe (right)"
#| fig-width: 4
#| fig-height: 3.5
...
You may also want to change the colors that are used in the scale. For continuous data, see Recipe @sec-recipe-colors-palette-continuous. For discrete data, see Recipe @sec-recipe-colors-palette-discrete and Recipe @sec-recipe-colors-palette-discrete-manual.


## Using a Colorblind-Friendly Palette {#sec-recipe-colors-palette-discrete-colorblind}
...
Here is an example from the introduction page to viridis (https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html; Figure @fig-colors-palette-viridis):

```{r}
#| label: fig-colors-palette-viridis
#| warning: false
#| echo: false
#| fig-cap: "Example of viridis color palette"
#| fig-height: 5
#| fig-width: 5
...
The viridis color scales can be implemented for data that is both continuous and discrete in nature. You will need to add `scale_fill_viridis_c()` to your plot if your data is continuous. If your data is discrete you will need to use `scale_fill_viridis_d()` instead, as in Figure @fig-colors-palette-discrete-colorblind-viridis below:

```{r}
#| label: fig-colors-palette-discrete-colorblind-viridis
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A plot with the colorblind-friendly viridis palette"
...
## Using a Different Palette for a Discrete Variable {#sec-recipe-colors-palette-discrete}
...
Use one of the scales listed in Table @tbl-table-discrete-fill-and-color-scales.

: Discrete fill and color scales {#tbl-table-discrete-fill-and-color-scales}
...
In the example here we'll use the default palette (hue), a viridis palette, and a ColorBrewer palette (Figure @fig-colors-palette-discrete):

```{r}
#| label: fig-colors-palette-discrete
#| message: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Default palette (using hue; top); A viridis palette (middle); A ColorBrewer palette (bottom)"
library(gcookbook)  # Load gcookbook for the uspopage data set
...
With `scale_fill_hue()`, the colors are taken from around the color wheel in the HCL (hue-chroma-lightness) color space. The default lightness value is 65 on a scale from 0–100. This is good for filled areas, but it's a bit light for points and lines. To make the colors darker for points and lines, as in Figure @fig-colors-palette-lightness (right), set the value of `l` (luminance/lightness):

```{r}
#| label: fig-colors-palette-lightness
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Points with default lightness (left); With lightness set to 45 (right)"
#| fig-width: 4
#| fig-height: 3.5
...
The viridis package provides a number of color scales that make it easy to see differences across your data. See Recipe @sec-recipe-colors-palette-discrete-colorblind for more details and examples.

The ColorBrewer package provides a number of palettes. You can generate a graphic showing all of them, as shown in Figure @fig-colors-palette-brewer:

```{r}
#| label: fig-colors-palette-brewer
#| echo: false
#| fig-cap: "All the ColorBrewer palettes"
#| fig-height: 8
#| fig-width: 8
...
display.brewer.all()
```
The ColorBrewer palettes can be selected by name. For example, this will use the "Oranges" palette (Figure @fig-colors-palette-brewer-name):

```{r}
#| label: fig-colors-palette-brewer-name
#| fig-cap: "Using a named ColorBrewer palette"
#| fig-width: 4
#| fig-height: 3.5
hw_splot +
...
You can also use a palette of greys. This is useful for print when the output is in black and white. The default is to start at 0.2 and end at 0.8, on a scale from 0 (black) to 1 (white), but you can change the range, as shown in Figure @fig-colors-palette-grey.

```{r}
#| label: fig-colors-palette-grey
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Using the default grey palette (left); A different grey palette (right)"
#| fig-width: 4
#| fig-height: 3.5
hw_splot +
  scale_colour_grey()
...
See Recipe @sec-recipe-legend-reverse for more information about reversing the legend.

To select colors manually, see Recipe @sec-recipe-colors-palette-discrete-manual.
...
## Using a Manually Defined Palette for a Discrete Variable {#sec-recipe-colors-palette-discrete-manual}
...
In the example here, we'll manually define colors by specifying values with `scale_colour_manual()` (Figure @fig-colors-palette-discrete-manual). The colors can be named, or they can be specified with RGB values:

```{r}
#| label: fig-colors-palette-discrete-manual
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Scatter plot with named colors (top left); With slightly different RGB colors (top right); With colors from the viridis color scale (bottom)"
#| fig-width: 4
#| fig-height: 3.5
...
You may want to manually select colors based on the color scales in the viridis package, as described in Recipe @sec-recipe-colors-palette-discrete. You can do so by calling `viridis()` and passing it the number of discrete categories you have. This will generate the RGB hexadecimal values. You can similarly generate the RGB values for the other color scales in the viridis package: "magma", "plasma", "inferno", and "cividis".
...
## Using a Manually Defined Palette for a Continuous Variable {#sec-recipe-colors-palette-continuous}
...
In the example here, we'll specify the colors for a continuous variable using various gradient scales (Figure @fig-colors-palette-continuous). The colors can be named, or they can be specified with RGB values:


```{r}
#| label: fig-colors-palette-continuous
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| message: false
#| fig-cap: "Clockwise from top left: default colors, two-color gradient (black and white) with `scale_colour_gradient()`, three-color gradient with midpoint with `scale_colour_gradient2()`, four-color gradient with `scale_colour_gradientn()`"
#| fig-width: 4
#| fig-height: 3.5
library(gcookbook)  # Load gcookbook for the heightweight data set
...
Mapping continuous values to a color scale requires a continuously changing palette of colors. Table @tbl-table-continuous-color-scales. lists the continuous color and fill scales.

: Continuous fill and color scales {#tbl-table-continuous-color-scales}
...
If you want use a discrete (categorical) scale instead of a continuous one, you can recode your data into categorical values. See Recipe @sec-recipe-dataprep-recode-continuous.


## Coloring a Shaded Region Based on Value {#sec-recipe-colors-area-value}
...
Once we've categorized the values as positive or negative, we can make the plot, mapping valence to the fill color, as shown in Figure @fig-colors-area-value:

```{r}
#| label: fig-colors-area-value
#| fig-cap: "Mapping valence to fill color-notice the red area under the zero line around 1950"
#| fig-width: 10
...
Now we can plot the interpolated data (Figure @fig-colors-area-value-interpolated). This time we'll make a few adjustments -- we'll make the shaded regions partially transparent, change the colors, remove the legend, and remove the padding on the left and right sides:

```{r}
#| label: fig-colors-area-value-interpolated
#| fig-cap: "Shaded regions with interpolated data"
#| fig-width: 10
