...
# Annotations {#sec-chapter-annotate}
...
## Adding Text Annotations {#sec-recipe-annotate-text}
...
Use `annotate()` and a text geom (Figure @fig-annotate-text):

```{r}
#| label: fig-annotate-text
#| fig-cap: "Text annotations"
#| fig-width: 4
#| fig-height: 4
...
Other text properties can be specified, as shown in Figure @fig-annotate-text-properties:

```{r}
#| label: fig-annotate-text-properties
#| fig-cap: "Modified text properties"
#| fig-width: 4
#| fig-height: 4
p +
  annotate("text", x = 3, y = 48, label = "Group 1",
...
Be careful not to use `geom_text()` when you want to add individual text objects. While `annotate(geom = "text")` will add a single text object to the plot, `geom_text()` will create many text objects based on the data, as discussed in Recipe @sec-recipe-scatter-labels.

If you use `geom_text()`, the text will be heavily overplotted on the same location, with one copy per data point:

```{r}
#| label: fig-annotate-text-geom-overplot
#| fig-cap: "Overplotting one of the labels -- both should be 90\\% transparent"
#| fig-width: 4
#| fig-height: 4
p +
  # Normal
...
In Figure @fig-annotate-text-geom-overplot, each text label is 90% transparent, making it clear which one is overplotted. The overplotting can lead to output with aliased (jagged) edges when outputting to a bitmap.

If the axes are continuous, you can use the special values `Inf` and `-Inf` to place text annotations at the edge of the plotting area, as shown in Figure @fig-annotate-text-edge. You will also need to adjust the position of the text relative to the corner using `hjust` and `vjust` -- if you leave them at their default values, the text will be centered on the edge. It may take a little experimentation with these values to get the text positioned to your liking:

```{r}
#| label: fig-annotate-text-edge
#| fig-cap: "Text positioned at the edge of the plotting area"
#| fig-width: 4
#| fig-height: 4
p +
...
See Recipe @sec-recipe-scatter-labels for making a scatter plot with text.

For more on controlling the appearance of the text, see Recipe @sec-recipe-appearance-text-appearance.


## Using Mathematical Expressions in Annotations {#sec-recipe-annotate-text-math}
...
Use `annotate(geom = "text")` with `parse = TRUE` (Figure @fig-annotate-text-math):

```{r}
#| label: fig-annotate-text-math
#| fig-cap: "Annotation with mathematical expressions"
# A normal curve
...
See Recipe @sec-recipe-scatter-fitlines-text for adding regression coefficients to a graph.

For using other fonts in mathematical expressions, see Recipe @sec-recipe-output-fonts-pdf.

## Adding Lines {#sec-recipe-annotate-lines}
...
For horizontal and vertical lines, use `geom_hline()` and `geom_vline()`, and for angled lines, use `geom_abline()` (Figure @fig-annotate-lines). For this example, we'll use the `heightweight` data set:

```{r}
#| label: fig-annotate-lines
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "horizontal and vertical lines (left); angled line (right)"
...
Here we'll take the average height for males and females and store it in a data frame, `hw_means`. Then we'll draw a horizontal line for each, and set the `linetype` and `size` (Figure @fig-annotate-lines-multi):

```{r}
#| label: fig-annotate-lines-multi
#| fig-cap: "Multiple lines, drawn at the mean of each group"
library(dplyr)
...
For adding regression lines, see Recipes Recipe @sec-recipe-scatter-fitlines and Recipe @sec-recipe-scatter-fitlines-model.

Lines are often used to indicate summarized information about data. See Recipe @sec-recipe-dataprep-summarize for more on how to summarize data by groups.


## Adding Line Segments and Arrows {#sec-recipe-annotate-segment}
...
Use `annotate("segment")`. In this example, we'll use the climate data set and use a subset of data from the Berkeley source (Figure @fig-annotate-segment):

```{r}
#| label: fig-annotate-segment
#| fig-cap: "Line segment annotation"
#| fig-width: 10
#| fig-height: 3
...
It's possible to add arrowheads or flat ends to the line segments, using `arrow()` from the grid package. In this example, we'll do both (Figure @fig-annotate-segment-arrows):

```{r}
#| label: fig-annotate-segment-arrows
#| fig-cap: "Line segments with arrow heads"
#| fig-width: 10
#| fig-height: 3
library(grid)
p +
...
## Adding a Shaded Rectangle {#sec-recipe-annotate-rect}
...
Use `annotate("rect")` (Figure @fig-annotate-rect):

```{r}
#| label: fig-annotate-rect
#| fig-cap: "A shaded rectangle"
#| fig-width: 10
#| fig-height: 3
library(gcookbook) # Load gcookbook for the climate data set
...
## Highlighting an Item {#sec-recipe-annotate-highlight}
...
Then we'll plot this data with specified colors, and hiding the legend (Figure @fig-annotate-highlight):

```{r}
#| label: fig-annotate-highlight
#| fig-cap: "Highlighting one item"
#| fig-width: 4
#| fig-height: 4
...
See Chapter @sec-chapter-colors for more information about specifying colors.

For more information about removing the legend, see Recipe @sec-recipe-legend-remove.


## Adding Error Bars {#sec-recipe-annotate-error-bar}
...
Use `geom_errorbar()` and map variables to the values for `ymin` and `ymax`. Adding the error bars is done the same way for bar graphs and line graphs, as shown in Figure @fig-annotate-errorbar-basic (notice that default *y* range is different for bars and lines, though):

```{r}
#| label: fig-annotate-errorbar-basic
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "error bars on a bar graph (left); on a line graph (right)"
...
For a bar graph with groups of bars, the error bars must also be *dodged*; otherwise, they'll have the exact same *x* coordinate and won't line up with the bars. (See Recipe @sec-recipe-bar-graph-grouped-bar for more information about grouped bars and dodging.)
...
The default dodge width for `geom_bar()` is 0.9, and you'll have to tell the error bars to be dodged the same width. If you don't specify the dodge width, it will default to dodging by the width of the error bars, which is usually less than the width of the bars (Figure @fig-annotate-errorbar-group-bar):

```{r}
#| label: fig-annotate-errorbar-group-bar
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "error bars on a grouped bar graph without dodging width specified (left); with dodging width specified (right)"
...
Additionally, you should dodge all the geometric elements so that they will align with the error bars, as shown in Figure @fig-annotate-errorbar-group-line-dodge:

```{r}
#| label: fig-annotate-errorbar-group-line-dodge
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Error bars on a line graph, dodged so they don't overlap"
...
See Recipe @sec-recipe-bar-graph-grouped-bar for more about creating grouped bar graphs, and Recipe @sec-recipe-line-graph-multiple-line for more about creating line graphs with multiple lines.

See Recipe @sec-recipe-dataprep-summarize-se for calculating summaries with means, standard deviations, standard errors, and confidence intervals.

See Recipe @sec-recipe-line-graph-region for adding a confidence region when the data has a higher density along the x-axis.


## Adding Annotations to Individual Facets {#sec-recipe-annotate-facet}
...
Create a new data frame with the faceting variable(s), and a value to use in each facet. Then use `geom_text()` with the new data frame (Figure @fig-annotate-facet):

```{r}
#| label: fig-annotate-facet
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Top: different annotations in each facet; bottom: the same annotation in each facet"
#| fig-width: 8
#| fig-height: 3
...
See Recipe @sec-recipe-annotate-text-math for more about using math expressions in plots.

If you want to make prediction lines from your own model objects, instead of having ggplot2 do it for you with `stat_smooth()`, see Recipe @sec-recipe-scatter-fitlines-model-multi.
