# Getting Your Data into Shape {#sec-chapter-dataprep}
...
The recipes in this chapter will often use packages from the *tidyverse*. For a little background about the tidyverse, see the introduction section of Chapter @sec-chapter-r-basics. I will also show how to do many of the same tasks using base R, because in some situations it is important to minimize the number of packages you use, and because it is useful to be able to understand code written for base R.

> **Note**
>
> The `%>%` symbol, also known as the pipe operator, is used extensively in this chapter. If you are not familiar with it, see Recipe @sec-recipe-r-basics-pipe.
...
## Creating a Data Frame {#sec-recipe-dataprep-create-dataframe}
...
## Getting Information About a Data Structure {#sec-recipe-dataprep-info-data}
...
## Adding a Column to a Data Frame {#sec-recipe-dataprep-add-col}
...
## Deleting a Column from a Data Frame {#sec-recipe-dataprep-delete-col}
...
Recipe @sec-recipe-dataprep-subset for more on getting a subset of a data frame.

See `?select` for more ways to drop and keep columns.


## Renaming Columns in a Data Frame {#sec-recipe-dataprep-rename-col}
...
## Reordering Columns in a Data Frame {#sec-recipe-dataprep-reorder-col}
...
## Getting a Subset of a Data Frame {#sec-recipe-dataprep-subset}
...
If you want only the `Year` and `Anomaly10y` columns, use `select()`, as we did in @sec-recipe-dataprep-delete-col:
...
This row filtering can also be combined with the column selection from @sec-recipe-dataprep-delete-col:
...
## Changing the Order of Factor Levels {#sec-recipe-dataprep-factor-reorder}
...
To reorder a factor based on the value of another variable, see Recipe @sec-recipe-dataprep-factor-reorder-value.

Reordering factor levels is useful for controlling the order of axes and legends. See Recipes Recipe @sec-recipe-axis-order and Recipe @sec-recipe-legend-order for more information.


## Changing the Order of Factor Levels Based on Data Values {#sec-recipe-dataprep-factor-reorder-value}
...
The usefulness of `reorder()` might not be obvious from just looking at the raw output. Figure @fig-dataprep-factor-reorder-value shows three plots made with `reorder()`. In these plots, the order in which the items appear is determined by their values.

```{r}
#| label: fig-dataprep-factor-reorder-value
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", "", ""]
#| layout-ncol: 3
#| fig-cap: "Original data (left); Reordered by the mean of each group (middle); Reordered by the median of each group (right)"
#| fig-height: 2.5
#| fig-width: 3
...
Reordering factor levels is also useful for controlling the order of axes and legends. See Recipes @sec-recipe-axis-order and @sec-recipe-legend-order for more information.


## Changing the Names of Factor Levels {#sec-recipe-dataprep-factor-rename}
...
If, instead of a factor, you have a character vector with items to rename, see Recipe @sec-recipe-dataprep-character-rename.


## Removing Unused Levels from a Factor {#sec-recipe-dataprep-factor-droplevels}
...
## Changing the Names of Items in a Character Vector {#sec-recipe-dataprep-character-rename}
...
If, instead of a character vector, you have a factor with levels to rename, see Recipe @sec-recipe-dataprep-factor-rename.


## Recoding a Categorical Variable to Another Categorical Variable {#sec-recipe-dataprep-recode-categorical}
...
Here, we combined two of the factor levels and put the result into a new column. If you simply want to rename the levels of a factor, see Recipe @sec-recipe-dataprep-factor-rename.
...
For more on renaming factor levels, see Recipe @sec-recipe-dataprep-factor-rename.

See Recipe @sec-recipe-dataprep-recode-continuous for recoding continuous values to categorical values.


## Recoding a Continuous Variable to a Categorical Variable {#sec-recipe-dataprep-recode-continuous}
...
To recode a categorical variable to another categorical variable, see Recipe @sec-recipe-dataprep-recode-categorical.


## Calculating New Columns From Existing Columns {#sec-recipe-dataprep-calculate}
...
See Recipe @sec-recipe-dataprep-calculate-group for how to perform group-wise transformations on data.


## Calculating New Columns by Groups {#sec-recipe-dataprep-calculate-group}
...
The before and after results are shown in Figure @fig-dataprep-calculate-group:

```{r}
#| label: fig-dataprep-calculate-group
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Before normalizing (left); After normalizing (right)"
...
To summarize data by groups, see Recipe @sec-recipe-dataprep-summarize.


## Summarizing Data by Groups {#sec-recipe-dataprep-summarize}
...
#### Missing combinations {#sec-_missing_combinations}

If there are any empty combinations of the grouping variables, they will not appear in the summarized data frame. These missing combinations can cause problems when making graphs. To illustrate, we'll remove all entries that have levels `c52` and `d21`. The graph on the left in Figure @fig-dataprep-summarize-missing-combo shows what happens when there's a missing combination in a bar graph:

```{r}
#| label: fig-dataprep-summarize-missing-combo-1
#| eval: false
...
To fill in the missing combination (Figure @fig-dataprep-summarize-missing-combo, right), use the `complete()` function from the tidyr package -- which is also part of the tidyverse. Also, the grouping for `c2a` must be removed, with `ungroup()`; otherwise it will return too many rows.

```{r}
#| label: fig-dataprep-summarize-missing-combo-2
#| eval: false
library(tidyr)
c2b <- c2a %>%
  ungroup() %>%
...
```{r}
#| label: fig-dataprep-summarize-missing-combo
#| ref-label: !expr c("fig-dataprep-summarize-missing-combo-1", "fig-dataprep-summarize-missing-combo-2")
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Bar graph with a missing combination (left); With missing combination filled (right)"
#| fig-width: 4
#| fig-height: 3
#| warning: false
```
...
If you want to calculate standard errors and confidence intervals, see Recipe @sec-recipe-dataprep-summarize-se.

See Recipe @sec-recipe-distribution-boxplot-mean for an example of using stat_summary() to calculate means and overlay them on a graph.

To perform transformations on data by groups, see Recipe @sec-recipe-dataprep-calculate-group.



## Summarizing Data with Standard Errors and Confidence Intervals {#sec-recipe-dataprep-summarize-se}
...
#### Confidence Intervals {#sec-_confidence_intervals}
...
See Recipe @sec-recipe-annotate-error-bar to use the values calculated here to add error bars to a graph.



## Converting Data from Wide to Long {#sec-recipe-dataprep-wide-to-long}
...
See Recipe @sec-recipe-dataprep-long-to-wide to do conversions in the other direction, from long to wide.

See the `stack()` function for another way of converting from wide to long.


## Converting Data from Long to Wide {#sec-recipe-dataprep-long-to-wide}
...
See Recipe @sec-recipe-dataprep-wide-to-long to do conversions in the other direction, from wide to long.

See the `unstack()` function for another way of converting from long to wide.


## Converting a Time Series Object to Times and Values {#sec-recipe-dataprep-timeseries}
