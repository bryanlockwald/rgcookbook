```{r}
#| label: setup-ggplotappendix
#| include: false
library(ggplot2)
library(dplyr)
```

# Understanding ggplot2 {#sec-chapter-ggplot2}
...
The following will make a simple grouped bar plot, with the `A`s going along the x-axis and the bars grouped by the `B`s (Figure @fig-ggplot-base-bar):


```{r}
#| label: fig-ggplot-base-bar
#| fig-cap: "A bar plot made with `barplot()`"
...
With the restructured data, we can create the plot the same way as before (Figure @fig-ggplot-base-bar-transpose):

```{r}
#| label: fig-ggplot-base-bar-transpose
#| fig-cap: "A bar plot with transposed data"
barplot(t(simpledat), beside=TRUE)
```

Another thing we might want to do is to represent the data with lines instead of bars, as shown in Figure @fig-ggplot-base-line. To do this with base graphics, we need to use a completely different set of commands. First we call `plot()`, which tells R to create a new plot and draw a line for one row of data. Then we tell it to draw a second row with `lines()`:


```{r}
#| label: fig-ggplot-base-line
#| fig-cap: "A line graph made with `plot()` and `lines()`"
...
This represents the same information, but with a different structure. Another term for it is *tidy data*, where each row represents one observation. There are advantages and disadvantages to this format, but on the whole, it makes things simpler when dealing with complicated data sets. See Recipes Recipe @sec-recipe-dataprep-wide-to-long and Recipe @sec-recipe-dataprep-long-to-wide for information about converting between wide and long data formats.

To make the first grouped bar plot (Figure @fig-ggplot-ggplot-bar), we first have to load the ggplot2 package. Then we tell it to map `Aval` to the *x* position, with `x = Aval`, and `Bval` to the fill color, with `fill = Bval`. This will make the `A`s run along the x-axis and the `B`s determine the grouping. We also tell it to map value to the *y* position, or height, of the bars, with `y = value`. Finally, we tell it to draw bars with `geom_col()` (don't worry about the other details yet; we'll get to those later):


```{r}
#| label: fig-ggplot-ggplot-bar
#| fig-cap: "A bar graph made with `ggplot()` and `geom_col()`"
library(ggplot2)
ggplot(simpledat_long, aes(x = Aval, y = value, fill = Bval)) +
    geom_col(position = "dodge")
```

To switch things so that the `B`s go along the x-axis and the `A`s determine the grouping (Figure @fig-ggplot-ggplot-bar-swap), we simply swap the mapping specification, with `x = Bval` and `fill = Aval`. Unlike with base graphics, we don't have to change the data; we just change the commands for making the plot:


```{r}
#| label: fig-ggplot-ggplot-bar-swap
#| fig-cap: "Bar plot of the same data, but with `x` and `fill` mappings switched"
...
To change it to a line plot (Figure @fig-ggplot-ggplot-line), we'll change `geom_col()` to `geom_line()`. We'll also map `Bval` to the *line* color, with `colour`, instead of the *fill* colour (note the British spelling -- the author of ggplot2 is a Kiwi). Again, don't worry about the other details yet:


```{r}
#| label: fig-ggplot-ggplot-line
#| fig-cap: "A line graph made with `ggplot()` and `geom_line()`"
...
We can also map the variable `group` to the color of the points, by putting `aes()` inside the call to `geom_point()`, and specifying `colour = group` (Figure @fig-ggplot-scatter-color-mapped):


```{r}
#| label: fig-ggplot-scatter-color-mapped
#| fig-cap: "A scatter plot with a variable mapped to `colour`"
p +
...
Contrast this aesthetic *mapping* with aesthetic *setting*. This time, we won't use `aes()`; we'll just set the value of colour directly (Figure @fig-ggplot-scatter-color-set):

```{r}
#| label: fig-ggplot-scatter-color-set
#| fig-cap: "A scatter plot with colors set instead of mapped"
p +
  geom_point(colour = "blue")
```

We can also modify the *scales*; that is, the mappings from data to visual attributes. Here, we'll change the *x* scale so that it has a larger range (Figure @fig-ggplot-scatter-range):

```{r}
#| label: fig-ggplot-scatter-range
#| fig-cap: "A scatter plot with increased x range"
p +
  geom_point() +
...
```{r}
#| label: fig-ggplot-scatter-color-scale-manual
#| fig-cap: "A scatter plot with modified colors and a different palette"
p +
  geom_point(aes(colour = group)) +
