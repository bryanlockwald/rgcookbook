...
# Bar Graphs {#sec-chapter-bar-graph}
...
## Making a Basic Bar Graph {#sec-recipe-bar-graph-basic-bar}
...
Use `ggplot()` with `geom_col()` and specify what variables you want on the x- and y-axes (Figure @fig-bar-graph-basic-bar):

```{r}
#| label: fig-bar-graph-basic-bar
#| fig-cap: "Bar graph of values with a discrete x-axis"
...
When x is a continuous (or numeric) variable, the bars behave a little differently. Instead of having one bar at each actual x value, there is one bar at each possible x value between the minimum and the maximum, as in Figure @fig-bar-graph-basic-bar-cont. You can convert the continuous variable to a discrete variable by using `factor()`.

```{r}
#| label: fig-bar-graph-basic-bar-cont
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Bar graph of values with a continuous x-axis (left); With x variable converted to a factor (notice that the space for 6 is gone; right)"
#| fig-width: 3.5
#| fig-height: 3.5
...
In these examples, the data has a column for x values and another for y values. If you instead want the height of the bars to represent the *count* of cases in each group, see Recipe @sec-recipe-bar-graph-counts.

By default, bar graphs use a dark grey for the bars. To use a color fill, use `fill`. Also, by default, there is no outline around the fill. To add an outline, use `colour`. For Figure @fig-bar-graph-basic-bar-single-fill, we use a light blue fill and a black outline:

```{r}
#| label: fig-bar-graph-basic-bar-single-fill
#| fig-cap: "A single fill and outline color for all bars"
ggplot(pg_mean, aes(x = group, y = weight)) +
...
If you want the height of the bars to represent the count of cases in each group, see Recipe @sec-recipe-bar-graph-counts.

To reorder the levels of a factor based on the values of another variable, see Recipe @sec-recipe-dataprep-factor-reorder-value. To manually change the order of factor levels, see Recipe @sec-recipe-dataprep-factor-reorder.

For more information about using colors, see Chapter @sec-chapter-colors.


## Grouping Bars Together {#sec-recipe-bar-graph-grouped-bar}
...
We'll map `Date` to the *x* position and map `Cultivar` to the fill color (Figure @fig-bar-graph-grouped-bar):

```{r}
#| label: fig-bar-graph-grouped-bar
#| fig-cap: "Graph with grouped bars"
...
To add a black outline, use `colour = "black"` inside `geom_col()`. To set the colors, you can use `scale_fill_brewer()` or `scale_fill_manual()`. In Figure @fig-bar-graph-grouped-bar-outline we'll use the `Pastel1` palette from `RColorBrewer`:

```{r}
#| label: fig-bar-graph-grouped-bar-outline
#| fig-cap: "Grouped bars with black outline and a different color palette"
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
...
Note that if there are any missing combinations of the categorical variables, that bar will be missing, and the neighboring bars will expand to fill that space. If we remove the last row from our example data frame, we get Figure @fig-bar-graph-grouped-bar-missing:

```{r}
#| label: fig-bar-graph-grouped-bar-missing
#| fig-cap: "Graph with a missing bar-the other bar fills the space"
ce <- cabbage_exp[1:5, ]
ce
...
For more on using colors in bar graphs, see Recipe @sec-recipe-bar-graph-colors.

To reorder the levels of a factor based on the values of another variable, see Recipe @sec-recipe-dataprep-factor-reorder-value.


## Making a Bar Graph of Counts {#sec-recipe-bar-graph-counts}
...
Use `geom_bar()` without mapping anything to `y` (Figure @fig-bar-graph-count):

```{r}
#| label: fig-bar-graph-count
#| fig-cap: "Bar graph of counts"
...
In this example, the variable on the x-axis is discrete. If we use a continuous variable on the x-axis, we'll get a bar at each unique *x* value in the data, as shown in Figure @fig-bar-graph-count-continuous, left:

```{r}
#| label: fig-bar-graph-count-continuous
#| echo: false
#| message: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Bar graph of counts on a continuous axis (left); A histogram (right)"
#| fig-width: 6
#| fig-height: 3
# Bar graph of counts
...
The bar graph with a continuous x-axis is similar to a histogram, but not the same. A histogram is shown on the right of Figure @fig-bar-graph-count-continuous. In this kind of bar graph, each bar represents a unique *x* value, whereas in a histogram, each bar represents a *range* of *x* values.


### See Also

If, instead of having `ggplot()` count up the number of rows in each group, you have a column in your data frame representing the y values, use `geom_col()`. See Recipe @sec-recipe-bar-graph-basic-bar.

You could also get the same graphical output by calculating the counts before sending the data to `ggplot()`. See Recipe @sec-recipe-dataprep-summarize for more on summarizing data.

For more about histograms, see Recipe @sec-recipe-distribution-basic-hist.


## Using Colors in a Bar Graph {#sec-recipe-bar-graph-colors}
...
(Figure @fig-bar-graph-fill):

```{r}
#| label: fig-bar-graph-fill
#| fig-cap: "A variable mapped to fill"
#| fig-width: 6
#| fig-height: 3.5
...
The default colors aren't the most appealing, so you may want to set them using `scale_fill_brewer()` or `scale_fill_manual()`. With this example, we'll use the latter, and we'll set the outline color of the bars to black, with `colour="black"` (Figure @fig-bar-graph-fill-manual). Note that *setting* occurs outside of `aes()`, while *mapping* occurs within `aes()`:

```{r}
#| label: fig-bar-graph-fill-manual
#| fig-cap: "Graph with different colors, black outlines, and sorted by percentage change"
#| fig-width: 6
#| fig-height: 3.5
...
For more about using `reorder()`, see Recipe @sec-recipe-dataprep-factor-reorder-value.

For more information about using colors, see Chapter @sec-chapter-colors.


## Coloring Negative and Positive Bars Differently {#sec-recipe-bar-graph-color-neg}
...
Once we have the data, we can make the graph and map pos to the fill color, as in Figure @fig-bar-graph-color-neg. Notice that we use position="identity" with the bars. This will prevent a warning message about stacking not being well defined for negative numbers:

```{r}
#| label: fig-bar-graph-color-neg
#| fig-cap: "Different colors for positive and negative values"
#| fig-width: 10
#| fig-height: 2.5
#| out-width: "100%"
...
We can change the colors with `scale_fill_manual()` and remove the legend with `guide = FALSE`, as shown in Figure @fig-bar-graph-color-neg2. We'll also add a thin black outline around each of the bars by setting `colour` and specifying `size`, which is the thickness of the outline (in millimeters):

```{r}
#| label: fig-bar-graph-color-neg2
#| fig-cap: "Graph with customized colors and no legend"
#| fig-width: 10
#| fig-height: 2.5
#| out-width: "100%"
ggplot(climate_sub, aes(x = Year, y = Anomaly10y, fill = pos)) +
...
To change the colors used, see Recipes Recipe @sec-recipe-colors-palette-discrete and Recipe @sec-recipe-colors-palette-discrete-manual.

To hide the legend, see Recipe @sec-recipe-legend-remove.


## Adjusting Bar Width and Spacing {#sec-recipe-bar-graph-adjust-width}
...
To make the bars narrower or wider, set `width` in `geom_col()`. The default value is 0.9; larger values make the bars wider, and smaller values make the bars narrower (Figure @fig-bar-width).
...
For grouped bars, the default is to have no space between bars within each group. To add space between bars within a group, make width smaller and set the value for `position_dodge` to be larger than `width` (Figure @fig-bar-width-dodge).
...
```{r}
#| label: fig-bar-width-dodge
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Bar graph with narrow grouped bars (left); With space between the bars (right)"
#| fig-width: 4.5
#| fig-height: 3
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
  geom_col(width = 0.5, position = "dodge")
...
```{r}
#| label: fig-bar-width-dodge-explanation
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Same dodge width of 0.9, but different bar widths of 0.9 (left) and 0.2 (right)"
#| fig-width: 4.5
#| fig-height: 3
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
  geom_col(position = "dodge")
...
If you make the entire graph wider or narrower, the bar dimensions will scale proportionally. To see how this works, you can just resize the window in which the graphs appear. For information about controlling this when writing to a file, see Chapter @sec-chapter-output.


## Making a Stacked Bar Graph {#sec-recipe-bar-graph-stacked-bar}
...
Use `geom_bar()` and map a variable `fill`. This will put `Date` on the x-axis and use `Cultivar` for the fill color, as shown in Figure @fig-bar-graph-stacked-bar:

```{r}
#| label: fig-bar-graph-stacked-bar
#| fig-cap: "Stacked bar graph"
#| fig-height: 3.5
library(gcookbook) # Load gcookbook for the cabbage_exp data set
...
```{r}
#| label: fig-bar-graph-stacked-bar-revlevels
#| fig-cap: "Stacked bar graph with reversed legend order"
#| fig-height: 3.5
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
  geom_col() +
...
If you'd like to reverse the stacking order of the bars, as in Figure @fig-bar-graph-stacked-bar-revstack, use `position_stack(reverse = TRUE)`. You'll also need to reverse the order of the legend for it to match the order of the bars:

```{r}
#| label: fig-bar-graph-stacked-bar-revstack
#| fig-cap: "Stacked bar graph with reversed stacking order"
#| fig-height: 3.5
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
...
It's also possible to modify the column of the data frame so that the factor levels are in a different order (see Recipe @sec-recipe-dataprep-factor-reorder). Do this with care, since the modified data could change the results of other analyses.

For a more polished graph, we'll use `scale_fill_brewer()` to get a different color palette, and use `colour="black"` to get a black outline (Figure @fig-bar-graph-stacked-bar-colors):

```{r}
#| label: fig-bar-graph-stacked-bar-colors
#| fig-cap: "Stacked bar graph with reversed legend, new palette, and black outline"
#| fig-height: 3.5
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
  geom_col(colour = "black") +
  scale_fill_brewer(palette = "Pastel1")
...
## Making a Proportional Stacked Bar Graph {#sec-recipe-bar-graph-proportional-stacked-bar}
...
Use `geom_col(position = "fill")` (Figure @fig-bar-graph-prop-stacked-bar):

```{r}
#| label: fig-bar-graph-prop-stacked-bar
#| fig-cap: "Proportional stacked bar graph"
#| fig-height: 3.5
library(gcookbook) # Load gcookbook for the cabbage_exp data set
...
To make the output look a little nicer, you can change the color palette and add an outline. This is shown in (Figure @fig-bar-graph-prop-stacked-bar-final):

```{r}
#| label: fig-bar-graph-prop-stacked-bar-final
#| fig-cap: "Proportional stacked bar graph with reversed legend, new palette, and black outline"
#| fig-height: 3.5
ggplot(cabbage_exp, aes(x = Date, y = Weight, fill = Cultivar)) +
...
> **Note**
>
> You may have noticed that `cabbage_exp` and `ce` print out differently. This is because `cabbage_exp` is a regular data frame, while `ce` is a *tibble*, which is a data frame with some extra properties. The dplyr package creates tibbles. For more information, see Chapter @sec-chapter-dataprep.
...
For more on transforming data by groups, see Recipe @sec-recipe-dataprep-calculate-group.


## Adding Labels to a Bar Graph {#sec-recipe-bar-graph-labels}
...
Add `geom_text()` to your graph. It requires a mapping for x, y, and the text itself. By setting `vjust` (the vertical justification), it is possible to move the text above or below the tops of the bars, as shown in Figure @fig-bar-graph-label:

```{r}
#| label: fig-bar-graph-label
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Labels under the tops of bars (left); Labels above bars (right)"
#| fig-height: 3.5
library(gcookbook) # Load gcookbook for the cabbage_exp data set
...
Notice that when the labels are placed atop the bars, they may be clipped. To remedy this, see Recipe @sec-recipe-axes-range.
...
```{r}
#| label: fig-bar-graph-count-label
#| fig-cap: "Bar graph of counts with labels under the tops of bars"
#| fig-width: 4
#| fig-height: 3.5
...
```{r}
#| label: fig-bar-label-stacked
#| fig-cap: "Labels on stacked bars"
#| fig-height: 3.5
...
The result is shown in Figure @fig-bar-label-stacked.

When using labels, changes to the stacking order are best done by modifying the order of levels in the factor (see Recipe @sec-recipe-dataprep-factor-reorder) before taking the cumulative sum. The other method of changing stacking order, by specifying breaks in a scale, won't work properly, because the order of the cumulative sum won't be the same as the stacking order.

To put the labels in the middle of each bar (Figure @fig-bar-label-stacked-middle), there must be an adjustment to the cumulative sum, and the *y* offset in `geom_bar()` can be removed:

```{r}
#| label: fig-bar-label-stacked-middle
#| fig-cap: "Labels in the middle of stacked bars"
#| fig-height: 3.5
ce <- cabbage_exp %>%
  arrange(Date, rev(Cultivar))
...
For a more polished graph (Figure @fig-bar-label-stacked-final), we'll change the colors, add labels in the middle with a smaller font using `size`, add a "kg" suffix using `paste`, and make sure there are always two digits after the decimal point by using `format()`:

```{r}
#| label: fig-bar-label-stacked-final
#| fig-cap: "Customized stacked bar graph with labels"
#| fig-height: 3.5
ggplot(ce, aes(x = Date, y = Weight, fill = Cultivar)) +
  geom_col(colour = "black") +
...
To control the appearance of the text, see Recipe @sec-recipe-appearance-text-appearance.

For more on transforming data by groups, see Recipe @sec-recipe-dataprep-calculate-group.


## Making a Cleveland Dot Plot {#sec-recipe-bar-graph-dot-plot}
...
The simplest way to create a dot plot (as shown in Figure @fig-bar-graph-dotplot) is to use `geom_point()`:

```{r}
#| label: fig-bar-graph-dotplot
#| fig-cap: "Basic dot plot"
#| fig-width: 4
#| fig-height: 5
#| out-width: "50%"
...
In Figure @fig-bar-graph-dotplot the names are sorted alphabetically, which isn't very useful in this graph. Dot plots are often sorted by the value of the continuous variable on the horizontal axis.
...
It's also possible to swap the axes so that the names go along the x-axis and the values go along the y-axis, as shown in Figure @fig-bar-graph-dotplot-ordered-swap. We'll also rotate the text labels by 60 degrees:

```{r}
#| label: fig-bar-graph-dotplot-ordered-swap
#| fig-cap: "Dot plot with names on x-axis and values on y-axis"
#| fig-width: 6
#| fig-height: 4
#| out-width: "80%"
...
For more on changing the order of factor levels, see Recipe @sec-recipe-dataprep-factor-reorder. Also see Recipe @sec-recipe-dataprep-factor-reorder-value for details on changing the order of factor levels based on some other values.

For more on moving the legend, see Recipe @sec-recipe-legend-position. To hide grid lines, see Recipe @sec-recipe-appearance-hide-gridlines.
