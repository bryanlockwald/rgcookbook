...
# Controlling the Overall Appearance of Graphs {#sec-chapter-appearance}
...
## Setting the Title of a Graph {#sec-recipe-appearance-title}
...
Use `ggtitle()` to add a title, as shown in Figure @fig-appearance-title:


```{r}
#| label: fig-appearance-title
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Scatter plot with a title added (left); With a `\n` for a newline (right)"
...
```{r}
#| label: fig-appearance-title-inside
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Title with ggtitle and a negative `vjust` value (note the extra space above the plotting area) (left); With a text annotation at the top of the figure (right)"
...
## Changing the Appearance of Text {#sec-recipe-appearance-text-appearance}
...
To set the appearance of theme items such as the title, axis labels, and axis tick marks, use theme() and set the item with `element_text()`. For example, `axis.title.x` controls the appearance of the x-axis label and `plot.title` controls the appearance of the title text (Figure @fig-appearance-text-appearance-theme):



```{r}
#| label: fig-appearance-text-appearance-theme
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "`axis.title.x` (left); `plot.title` (right)"
library(gcookbook)  # Load gcookbook for the heightweight data set
...
To set the appearance of text geoms (text that's in the plot itself, with `geom_text()` or `annotate()`), set the text properties. For example (Figure @fig-appearance-text-appearance-geom):


```{r}
#| label: fig-appearance-text-appearance-geom
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "`annotate(\"text\")` (left); `geom_text()` (right)"
hw_plot +
...
There are differences in the parameters, as shown in Table @tbl-table-appearance-properties.

: Text properties of theme elements and text geoms {#tbl-table-appearance-properties}
...
The theme elements are listed in Table @tbl-table-appearance-theme-text-elements. Most of them are straightforward. Some are shown in Figure @fig-appearance-text-appearance-properties.

: Theme items that control text appearance in `theme()` {#tbl-table-appearance-theme-text-elements}
...
```{r}
#| label: fig-appearance-text-appearance-properties
#| fig-cap: "Aligning with `hjust` and `vjust`, and spacing with `lineheight`"
#| echo: false
#| fig-width: 8
#| fig-height: 4
ggplot(data.frame()) +
...
## Using Themes {#sec-recipe-appearance-theme}
...
To use a premade theme, add `theme_bw()` or another theme to your plot (Figure @fig-appearance-theme):



```{r}
#| label: fig-appearance-theme
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Scatter plot with `theme_grey()` (the default, top left); with `theme_bw()` (top right); with `theme_minimal()` (bottom left); with `theme_classic()` (bottom right)"
library(gcookbook)  # Load gcookbook for the heightweight data set
...
Another theme included in ggplot2 is `theme_void()`, which makes all plot elements blank and only shows your data (Figure @fig-appearance-theme-void. This is especially useful if you don't want any default theme settings, and instead want a blank slate on which to choose your own theme elements.


```{r}
#| label: fig-appearance-theme-void
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Scatter plot with `theme_void()`"
hw_plot +
  theme_void()
```
...
To modify a theme, see Recipe @sec-recipe-appearance-theme-modify.

To create your own themes, see Recipe @sec-recipe-appearance-theme-create.

See `?theme` to see all the available theme properties.


## Changing the Appearance of Theme Elements {#sec-recipe-appearance-theme-modify}
...
To modify a theme, add `theme()` with a corresponding `element_`*xx* object. These include `element_line`, `element_rect`, and `element_text`. The following code shows how to modify many of the commonly used theme properties (Figure @fig-appearance-theme-modify):

```{r}
#| label: fig-appearance-theme-modify
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Clockwise from top left: modifying theme properties for the plotting area, the legend, the facets, and the text items"
library(gcookbook)  # Load gcookbook for the heightweight data set
...
Many of the commonly used theme properties are shown in Table @tbl-table-appearance-theme-elements.


: Theme items that control text appearance in `theme()` {#tbl-table-appearance-theme-elements}
...
## Creating Your Own Themes {#sec-recipe-appearance-theme-create}
...
You can create your own theme by adding elements to an existing theme (Figure @fig-appearance-theme-save):

```{r}
#| label: fig-appearance-theme-save
#| fig-cap: "A modified default theme"
library(gcookbook) # Load gcookbook for the heightweight data set
...
The options for modifying themes are listed in Recipe @sec-recipe-appearance-theme-modify.


## Hiding Grid Lines {#sec-recipe-appearance-hide-gridlines}
...
The major grid lines (those that align with the tick marks) are controlled with `panel.grid.major`. The minor grid lines (the ones between the major lines) are controlled with `panel.grid.minor`. This will hide them both, as shown in Figure @fig-appearance-hide-gridlines (left):

```{r}
#| label: fig-appearance-hide-gridlines-1
#| eval: false
library(gcookbook)  # Load gcookbook for the heightweight data set
...
It's possible to hide just the vertical or horizontal grid lines, as shown in the middle and right-hand graphs in Figure @fig-appearance-hide-gridlines, with `panel.grid.major.x`, `panel.grid.major.y`, `panel.grid.minor.x`, and `panel.grid.minor.y`:

```{r}
#| label: fig-appearance-hide-gridlines-2
#| eval: false
...
```{r}
#| label: fig-appearance-hide-gridlines
#| ref-label: !expr c("fig-appearance-hide-gridlines-1", "fig-appearance-hide-gridlines-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "No grid lines (left); No vertical lines (middle); No horizontal lines (right)"
#| fig-height: 2.5
#| fig-width: 2.5
```
