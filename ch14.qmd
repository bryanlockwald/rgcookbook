...
# Output for Presentation {#sec-chapter-output}
...
## Outputting to PDF Vector Files {#sec-recipe-output-vector}
...
## Outputting to WMF Vector Files {#sec-recipe-output-vector-wmf}
...
## Editing a Vector Output File {#sec-recipe-output-edit-vector}
...
For high-quality print output, use at least 300 ppi. Figure @fig-output-bitmap-res shows portions of the same plot at different resolutions. In this example, we'll use 300 ppi and create a 4×4-inch PNG file:
...
```{r}
#| label: fig-output-bitmap-res
#| echo: false
#| fig-cap: "From left to right: PNG output at 72, 150, and 300 ppi (actual size)"
#| fig-width: 3.5
...
Changing the resolution affects the size (in pixels) of graphical objects like text, lines, and points. For example, a 6-by-6-inch image at 75 ppi has the same pixel dimensions as a 3-by-3-inch image at 150 ppi, but the appearance will be different, as shown in Figure @fig-output-bitmap-size. Both of these images are 450×450 pixels. When displayed on a computer screen, they may display at approximately the same size, as they do here.

```{r}
#| label: fig-output-bitmap-size
#| echo: false
#| fig-cap: "6×6 inch image at 75 ppi (left); 3×3 inch image at 150 ppi (right)"
#| fig-width: 6
knitr::include_graphics("fig/output-ppi-size.png")
```


## Using Fonts in PDF Files {#sec-recipe-output-fonts-pdf}
...
```{r}
#| label: fig-output-fonts-pdf
#| echo: false
#| fig-cap: "PDF output with embedded font Impact"
#| fig-width: 2.5
...
For more on controlling text appearance, see Recipe @sec-recipe-appearance-text-appearance.


## Using Fonts in Windows Bitmap or Screen Output {#sec-recipe-output-fonts-win}
...
The extrafont package can be used to create bitmap or screen output. The procedure is similar to using extrafont with PDF files (Recipe @sec-recipe-output-fonts-pdf). The one-time setup is almost the same, except that Ghostscript is not required:
...
Finally, you can create each output file or display graphs on screen, as in Figure @fig-output-fonts-png:
...
```{r}
#| label: fig-output-fonts-png
#| echo: false
#| fig-cap: "PNG output with font Georgia Italic"
#| fig-width: 2.5
...
## Combining Several Plots into the Same Graphic {#sec-recipe-combine-ggplot}
...
The patchwork package can be used to combine several ggplot2 plots into one graphic. (Figure @fig-output-patchwork).


```{r}
#| label: fig-output-patchwork
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Combining two plots together using `patchwork()`"
#| fig-height: 3.5
#| fig-width: 7
library(patchwork)
...
Patchwork also allows you to determine how you want to lay out the ggplots in relation to each other, by adding a `plot_layout()` call. You can use this call to determine the number of columns you want the pplots to be arranged in (Figure @fig-output-patchwork-cols), and the size of the plots (Figure @fig-output-patchwork-height):

```{r}
#| label: fig-output-patchwork-cols
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Using `plot_layout()` to specify that the plots should be arranged in 2 columns"
#| fig-height: 7
#| fig-width: 8
...
```{r}
#| label: fig-output-patchwork-height
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Using `plot_layout()` to specify the heights of each plot"
plot1 + plot2 +
