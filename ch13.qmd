...
# Miscellaneous Graphs {#sec-chapter-miscgraph}
...
## Making a Correlation Matrix {#sec-recipe-miscgraph-corrmatrix}
...
To plot the correlation matrix (Figure @fig-miscgraph-corrmatrix-basic), we'll use the corrplot package:

```{r}
#| label: fig-miscgraph-corrmatrix-basic
...
The `corrplot()` function has many, many options. Here is an example of how to make a correlation matrix with colored squares and black labels, rotated 45 degrees along the top (Figure @fig-miscgraph-corrmatrix-shade):

```{r}
#| label: fig-miscgraph-corrmatrix-shade
#| fig-cap: "Correlation matrix with colored squares and black, rotated labels"
#| fig-width: 7
#| fig-height: 7
corrplot(mcor, method = "shade", shade.col = NA, tl.col = "black", tl.srt = 45)
```

It may also be helpful to display labels representing the correlation coefficient on each square in the matrix. In this example we'll make a lighter palette so that the text is readable, and we'll remove the color legend, since it's redundant. We'll also order the items so that correlated items are closer together, using the `order="AOE"` (angular order of eigenvectors) option. The result is shown in Figure @fig-miscgraph-corrmatrix-shade-num:

```{r}
#| label: fig-miscgraph-corrmatrix-shade-num
#| fig-cap: "Correlation matrix with correlation coefficients and no legend"
#| fig-width: 7
#| fig-height: 7
...
Like many other standalone plotting functions, `corrplot()` has its own menagerie of options, which can't all be illustrated here. Table @tbl-table-options-for-corrplot lists some useful options.

: Options for `corrplot()` {#tbl-table-options-for-corrplot}
...
To create a scatter plot matrix, see Recipe @sec-recipe-scatter-splom.

For more on subsetting data, see Recipe @sec-recipe-dataprep-subset.


## Plotting a Function {#sec-recipe-miscgraph-function}
...
Use `stat_function()`. It's also necessary to give ggplot a dummy data frame so that it will get the proper *x* range. In this example we'll use `dnorm()`, which gives the density of the normal distribution (Figure @fig-miscgraph-function, left):

```{r}
#| label: fig-miscgraph-function-1
#| eval: false
...
Some functions take additional arguments. For example, `dt()`, the function for the density of the *t*-distribution, takes a parameter for degrees of freedom (Figure @fig-miscgraph-function, right). These additional arguments can be passed to the function by putting them in a list and giving the list to args:

```{r}
#| label: fig-miscgraph-function-2
#| eval: false
...
```{r}
#| label: fig-miscgraph-function
#| ref-label: !expr c("fig-miscgraph-function-1", "fig-miscgraph-function-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "The normal distribution (left); The *t*-distribution with `df=2` (right)"
```

It's also possible to define your own functions. It should take an *x* value for its first argument, and it should return a *y* value. In this example, we'll define a sigmoid function (Figure @fig-miscgraph-function-custom):

```{r}
#| label: fig-miscgraph-function-custom
#| fig-cap: "A user-defined function"
myfun <- function(xvar) {
    1 / (1 + exp(-xvar + 10))
}
...
For plotting predicted values from model objects (such as `lm` and `glm`), see Recipe @sec-recipe-scatter-fitlines-model.



## Shading a Subregion Under a Function Curve {#sec-recipe-miscgraph-function-shade}
...
Define a new wrapper function around your curve function, and replace out-of-range values with `NA`, as shown in Figure @fig-miscgraph-function-shade:

```{r}
#| label: fig-miscgraph-function-shade
#| fig-cap: "Function curve with a shaded region"
...
If you look very, very closely at the graph in Figure @fig-miscgraph-function-shade, you may see that the shaded region does not align exactly with the range we specified. This is because ggplot2 does a numeric approximation by calculating values at fixed intervals, and these intervals may not fall exactly within the specified range. As in Recipe @sec-recipe-miscgraph-function, we can improve the approximation by increasing the number of interpolated values with `stat_function(n = 200)`.



## Creating a Network Graph {#sec-recipe-miscgraph-graph}
...
Use the igraph package. To create a graph, pass a vector containing pairs of items to `graph()`, then plot the resulting object (Figure @fig-miscgraph-graph-basic):

```{r}
#| label: fig-miscgraph-graph-basic
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A directed graph (left); An undirected graph, with no vertex labels (right)"
#| fig-width: 4.5
#| fig-height: 4.5
...
It's also possible to create a graph from a data frame. The first two columns of the data frame are used, and each row specifies a connection between two nodes. In the next example (Figure @fig-miscgraph-graph-directed), we'll use the `madmen2` data set, which has this structure. We'll also use the Fruchterman-Reingold layout algorithm. The idea is that all the nodes have a magnetic repulsion from one another, but the edges between nodes act as springs, pulling the nodes together:

```{r}
#| label: fig-miscgraph-graph-directed
#| fig-cap: "A directed graph from a data frame, with the Fruchterman-Reingold algorithm"
#| fig-width: 5
#| fig-height: 5
...
It's also possible to make a directed graph from a data frame. The `madmen` data set has only one row for each pairing, since direction doesn't matter for an undirected graph. This time we'll use a circle layout (Figure @fig-miscgraph-graph-circle):

```{r}
#| label: fig-miscgraph-graph-circle
#| fig-cap: "A circular undirected graph from a data frame"
...
## Using Text Labels in a Network Graph {#sec-recipe-miscgraph-graph-label}
...
The vertices/nodes may have names, but these names are not used as labels by default. To set the labels, pass in a vector of names to `vertex.label` (Figure @fig-miscgraph-graph-label):

```{r}
#| label: fig-miscgraph-graph-label
#| fig-cap: "A network graph with labels"
#| fig-width: 6
#| fig-height: 6
library(igraph)
library(gcookbook) # For the data set
...
The properties of the edges can also be set, either with the `E()` function or by passing values to `edge.xxx` arguments (Figure @fig-miscgraph-graph-label-edge):
...
```{r}
#| label: fig-miscgraph-graph-label-edge
#| fig-cap: "A network graph with labeled and colored edges"
#| fig-width: 6
#| fig-height: 6
# View the edges
E(g)
...
## Creating a Heat Map {#sec-recipe-miscgraph-heatmap}
...
Now we can make the plot using `geom_tile()` or `geom_raster()` (Figure @fig-miscgraph-heatmap). Simply map one variable to `x`, one to `y`, and one to `fill`:
...
```{r}
#| label: fig-miscgraph-heatmap
#| echo: false
#| fig-cap: "A heat map-the grey squares represent `NA`s in the data"
#| fig-width: 13
#| fig-height: 2
...
> **Note**
>
> The results with `geom_tile()` and `geom_raster()` *should* look the same, but in practice they might appear different. See Recipe @sec-recipe-distribution-density2d for more information about this issue.

### Discussion

To better convey useful information, you may want to customize the appearance of the heat map. With this example, we'll reverse the y-axis so that it progresses from top to bottom, and we'll add tick marks every four years along the x-axis, to correspond with each presidential term. For the x and y scales, we remove the padding by using `expand=c(0, 0)`.  We'll also change the color scale using `scale_fill_gradient2()`, which lets you specify a midpoint color and the two colors at the low and high ends (Figure @fig-miscgraph-heatmap-custom):

```{r}
#| label: fig-miscgraph-heatmap-custom
#| fig-cap: "A heat map with customized appearance"
#| fig-width: 13
#| fig-height: 2
p +
  geom_tile() +
...
If you want to use a different color palette, see Recipe @sec-recipe-colors-palette-continuous.



## Creating a Three-Dimensional Scatter Plot {#sec-recipe-miscgraph-3d-scatter}
...
We'll use the rgl package, which provides an interface to the OpenGL graphics library for 3D graphics. To create a 3D scatter plot, as in Figure @fig-miscgraph-3d-scatter, use `plot3d()` and pass in a data frame where the first three columns represent *x*, *y*, and *z* coordinates, or pass in three vectors representing the *x*, *y*, and *z* coordinates.
...
```{r}
#| label: fig-miscgraph-3d-scatter
#| echo: false
#| fig-cap: "A 3D scatter plot"
...
In Figure @fig-miscgraph-3d-scatter2, we'll add vertical segments to help give a sense of the spatial positions of the points:
...
```{r}
#| label: fig-miscgraph-3d-scatter2
#| echo: false
#| fig-cap: "A 3D scatter plot with vertical lines for each point"
knitr::include_graphics("fig/FIG-MISCGRAPH-3D-SCATTER2.png")
```

It's possible to tweak the appearance of the background and the axes. In Figure @fig-miscgraph-3d-scatter3, we change the number of tick marks and add tick marks and axis labels to the specified sides:
...
```{r}
#| label: fig-miscgraph-3d-scatter3
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "3D scatter plot with axis ticks and labels repositioned"
...
## Adding a Prediction Surface to a Three-Dimensional Plot {#sec-recipe-miscgraph-3d-scatter-model}
...
With these utility functions defined, we can make a linear model from the data and plot it as a mesh along with the data, using the `surface3d()` function, as shown in Figure @fig-miscgraph-3d-scatter-model:
...
```{r}
#| label: fig-miscgraph-3d-scatter-model
#| echo: false
#| fig-cap: "A 3D scatter plot with a prediction surface"
knitr::include_graphics("fig/FIG-MISCGRAPH-3D-SCATTER-MODEL.png")
```

### Discussion

We can tweak the appearance of the graph, as shown in Figure @fig-miscgraph-3d-scatter-model2. We'll add each of the components of the graph separately:
...
```{r}
#| label: fig-miscgraph-3d-scatter-model2
#| echo: false
#| fig-cap: "Three-dimensional scatter plot with customized appearance"
...
## Saving a Three-Dimensional Plot {#sec-recipe-miscgraph-3d-save}
...
## Animating a Three-Dimensional Plot {#sec-recipe-miscgraph-3d-animate}
...
## Creating a Dendrogram {#sec-recipe-miscgraph-dendrogram}
...
Finally, we're ready to make the dendrogram, as shown in Figure @fig-miscgraph-dendrogram:

```{r}
#| label: fig-miscgraph-dendrogram
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A dendrogram (left); With text aligned (right)"
#| fig-width: 6
#| fig-height: 5
hc <- hclust(dist(c3))
...
First, we normalized the data using `scale()` with its default settings. You can scale your data differently, or not at all. (With this data set, *not* scaling the data will lead to GDP overwhelming the other variables, as shown in Figure @fig-miscgraph-dendrogram-unscaled.)

```{r}
#| label: fig-miscgraph-dendrogram-unscaled
#| fig-cap: "Dendrogram with unscaled data-notice the much larger Height values, which are largely due to the unscaled GDP values"
#| fig-width: 6
#| fig-height: 5
...
## Creating a Vector Field {#sec-recipe-miscgraph-vectorfield}
...
Now that we've taken a subset of the data, we can plot it, with arrowheads, as shown in Figure @fig-miscgraph-vectorfield-arrow:

```{r}
#| label: fig-miscgraph-vectorfield-arrow
#| fig-cap: "Vector field with arrowheads"
#| fig-width: 5
#| fig-height: 5
#| warning: false
...
Next, we'll map speed to colour. We'll also add a map of the United States and zoom in on the area of interest, as shown in the graph on the right in Figure @fig-miscgraph-vectorfield-aes,using `coord_cartesian()` (without this, the entire USA will be displayed):

```{r}
#| label: fig-miscgraph-vectorfield-aes-2
#| eval: false
# Get USA map data
usa <- map_data("usa")
...
```{r}
#| label: fig-miscgraph-vectorfield-aes
#| ref-label: !expr c("fig-miscgraph-vectorfield-aes-1", "fig-miscgraph-vectorfield-aes-2")
#| echo: false
#| message: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Vector field with speed mapped to alpha (left); With speed mapped to colour (right)"
#| fig-width: 6
#| fig-height: 5
#| warning: false
...
The `isabel` data set has three-dimensional data, so we can also make a faceted graph of the data, as shown in Figure @fig-miscgraph-vectorfield-facet. Because each facet is small, we will use a sparser subset than before:


```{r}
#| label: fig-miscgraph-vectorfield-facet
#| fig-cap: "Vector field of wind speeds, faceted on `z`"
#| fig-width: 9
#| fig-height: 6
#| warning: false
...
See Recipe @sec-recipe-axes-range for more information about zooming in on part of a graph.


## Creating a QQ Plot {#sec-recipe-miscgraph-qq}
...
Use `geom_qq()` and `geom_qq_line()` to compare to a normal distribution (Figure @fig-miscgraph-qq):

```{r}
#| label: fig-miscgraph-qq
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "QQ plot of height, which is close to normally distributed (left); QQ plot of age, which is not normally distributed (right)"
library(gcookbook) # For the data set
...
## Creating a Graph of an Empirical Cumulative Distribution Function {#sec-recipe-miscgraph-ecdf}
...
Use `stat_ecdf()` (Figure @fig-miscgraph-ecdf):

```{r}
#| label: fig-miscgraph-ecdf
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "ECDF of height (left); ECDF of age (right)"
library(gcookbook) # For the data set
...
## Creating a Mosaic Plot {#sec-recipe-miscgraph-mosaic}
...
The three dimensions are `Admit`, `Gender`, and `Dept`. To visualize the relationships between the variables (Figure @fig-miscgraph-mosaic), use `mosaic()` and pass it a formula with the variables that will be used to split up the data:

```{r}
#| label: fig-miscgraph-mosaic
#| fig-cap: "Mosaic plot of UC-Berkeley admissions data-the area of each rectangle is proportional to the number of cases in that cell"
#| fig-width: 5
#| fig-height: 9
...
Another way of looking at the data is to split first by department, then gender, then admission status, as in Figure @fig-miscgraph-mosaic2. This makes the admission status the last variable that is partitioned, so that *after* partitioning by department and gender, the admitted and rejected cells for each group are right next to each other:

```{r}
#| label: fig-miscgraph-mosaic2
#| fig-cap: "Mosaic plot with a different variable splitting order: first department, then gender, then admission status"
#| fig-width: 14
#| fig-height: 4
...
We can also use different splitting directions, as shown in Figure @fig-miscgraph-mosaic3 and Figure @fig-miscgraph-mosaic4:

```{r}
#| label: fig-miscgraph-mosaic3
#| fig-cap: "Splitting Dept vertically, Gender vertically, and Admit horizontally"
#| fig-width: 12
#| fig-height: 3
...
```{r}
#| label: fig-miscgraph-mosaic4
#| fig-cap: "Splitting Dept vertically, Gender horizontally, and Admit horizontally"
#| fig-width: 12
#| fig-height: 5
...
In Figure @fig-miscgraph-mosaic2 and Figure @fig-miscgraph-mosaic3, you can see that within each department, admission rates were approximately equal between men and women. You can also see that departments with higher admission rates (A and B) were very imbalanced in the gender ratio of applicants: far more men applied to these departments than did women. As you can see, partitioning the data in different orders and directions can bring out different aspects of the data. In Figure @fig-miscgraph-mosaic3, as in Figure @fig-miscgraph-mosaic2, it’s easy to compare male and female admission rates within each department and across departments. Splitting `Dept` vertically, `Gender` horizontally, and `Admit` horizontally, as in Figure @fig-miscgraph-mosaic4, makes it difficult to compare male and female admission rates within each department, but it is easy to compare male and female application rates across departments.
...
## Creating a Pie Chart {#sec-recipe-miscgraph-pie}
...
Use the `pie()` function. In this example (Figure @fig-miscgraph-pie), we'll use the survey data set from the MASS library:

```{r}
#| label: fig-miscgraph-pie
#| fig-cap: "A pie chart"
...
## Creating a Map {#sec-recipe-miscgraph-map}
...
It's possible to get data for specific regions from a particular map (Figure @fig-miscgraph-map-regions):

```{r}
#| label: fig-miscgraph-map-regions
#| fig-cap: "Specific regions from the world map"
#| fig-width: 8
#| fig-height: 5
...
If there is a separate map available for a region, such as `nz` (New Zealand), that map data will be at a higher resolution than if you were to extract it from the world map, as shown in Figure @fig-miscgraph-map-resolution:


```{r}
#| label: fig-miscgraph-map-resolution
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "New Zealand data taken from `world` map (left); Data from `nz` map (right)"
...
## Creating a Choropleth Map {#sec-recipe-miscgraph-choropleth}
...
Once the data is in the correct format, it can be plotted (Figure @fig-miscgraph-choropleth), mapping one of the columns with data values to fill:

```{r}
#| label: fig-miscgraph-choropleth
#| fig-cap: "A map with a variable mapped to fill"
#| fig-width: 7
#| fig-height: 5
...
The preceding example used the default color scale, which goes from dark to light blue. If you want to show how the values diverge from some middle value, you can use `scale_fill_gradient2()`, or `scale_fill_viridis_c()` as shown in Figure @fig-miscgraph-choropleth-midpoint:

```{r}
#| label: fig-miscgraph-choropleth-midpoint
#| fig-cap: "With a diverging color scale"
#| fig-width: 7
#| fig-height: 5
#| fig-subcap: ["", ""]
#| layout-ncol: 2
...
The previous example mapped continuous values to fill, but we could just as well use discrete values. It's sometimes easier to interpret the data if the values are discretized. For example, we can categorize the values into quantiles and show those quantiles, as in Figure @fig-miscgraph-choropleth-discrete:


```{r}
#| label: fig-miscgraph-choropleth-discrete
#| fig-cap: "Choropleth map with discretized data"
#| fig-width: 7
#| fig-height: 5
...
For an example of data overlaid on a map, see Recipe @sec-recipe-miscgraph-vectorfield.

For more on using continuous colors, see Recipe @sec-recipe-colors-palette-continuous.



## Making a Map with a Clean Background {#sec-recipe-miscgraph-map-background}
...
Use `theme_void()` (Figure @fig-miscgraph-map-background). In this example, we'll add it to one of the choropleths we created in Recipe @sec-recipe-miscgraph-choropleth:

```{r}
#| label: fig-miscgraph-map-background
#| fig-cap: "A map with a clean background"
#| fig-width: 7
#| fig-height: 5
ggplot(crimes, aes(map_id = state, fill = Assault_q)) +
  geom_map(map = states_map, colour = "black") +
...
Load the shapefile using `st_read()` from the sf package, then plot it with `geom_sf()` (Figure @fig-miscgraph-map-shapefile):

```{r}
#| label: fig-miscgraph-map-shapefile
#| fig-cap: "A map created from a shapefile"
#| fig-width: 8
#| fig-height: 6
library(sf)
...
Columns in the data can be mapped to aesthetics like fill. For example, we can mapp the `ENGTYPE_2` column to fill, as shown in Figure @fig-miscgraph-map-shapefile-2:


```{r}
#| label: fig-miscgraph-map-shapefile-2
#| fig-cap: "With a column mapped to `fill`"
#| fig-width: 8
#| fig-height: 6
