# Scatter Plots {#sec-chapter-scatter}

```{r}
#| label: setup-ch05
#| include: false
library(MASS)        # load MASS first so dplyr functions take precedence
library(ggplot2)
library(dplyr)
# Pin dplyr::select in .GlobalEnv so it wins over MASS::select even if
# library(MASS) is called again later in the chapter.
select <- dplyr::select
```
...
## Making a Basic Scatter Plot {#sec-recipe-scatter-basic-scatter}
...
We will use the `heightweight` data set. There are a number of columns in this data set, but we'll only use two in this example (Figure @fig-scatter-basic):
...
Instead of points, you can use different shapes for your scatter plot by using the `shape` aesthetic. A common alternative to the default solid circles (shape #19) is hollow ones (#21), as seen in Figure @fig-scatter-basic-shape-size (left):
...
The size of the points can be controlled with the `size` aesthetic. The default value of size is 2 (`size = 2`). The following code will set `size = 1.5` to create smaller points (Figure @fig-scatter-basic-shape-size, right):
...
```{r}
#| label: fig-scatter-basic-shape-size
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Scatter plot with hollow circles (shape 21, left); With smaller points (right)"
#| fig-width: 4
#| fig-height: 4
...
## Grouping Points Together using Shapes or Colors {#sec-recipe-scatter-grouped-scatter}
...
We can use the aesthetics of `colour` or `shape` to visually differentiate the data points belonging to different categories of `sex`. We do this by mapping `sex` to one of the aesthetics `colour` or `shape` (Figure@fig-scatter-shape-color):

```{r}
#| label: fig-scatter-shape-color
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Grouping points by a variable mapped to colour (left), or to shape (right)"
...
It is possible to map a variable to both `shape` and `colour`, or, if you have multiple grouping variables, to map each grouping variable to a different aesthetic. Here, we'll map the variable `sex` to both `shape` and `colour` aesthetics (Figure @fig-scatter-shape-color-both, left):
...
You may want to use different shapes and colors than are given by the default settings. You can select other shapes for the grouping variables using `scale_shape_manual()`, and select other colors using `scale_colour_brewer()` or `scale_colour_manual()`. (Figure @fig-scatter-shape-color-both, right):
...
```{r}
#| label: fig-scatter-shape-color-both
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Mapping to both shape and colour (left); With manually set shapes and colors (right)"
ggplot(heightweight, aes(x = ageYear, y = heightIn, shape = sex, colour = sex)) +
  geom_point()
...
To use different shapes, see Recipe @sec-recipe-scatter-shapes.

For more on using different colors, see Chapter @sec-chapter-colors.


## Using Different Point Shapes {#sec-recipe-scatter-shapes}
...
You can set the shape of all the data points at once (Figure @fig-scatter-shapes, left) by setting a shape in `geom_point()`:
...
```{r}
#| label: fig-scatter-shapes
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Scatter plot with the shape aesthetic set to a custom value (left); With a variable mapped to shape, using a custom shape palette (right)"
#| fig-width: 8.5
#| fig-height: 3.5
library(gcookbook) # Load gcookbook for the heightweight data set
...
Figure @fig-scatter-shapes-chart shows the shapes that are already built into R. Some of the point shapes (1–14) only have an outline; some (15–20) have solid fill; and some (21–25) have an outline and fill that can be controlled separately. You can also use characters for points.
...
```{r}
#| label: fig-scatter-shapes-chart
#| echo: false
#| fig-cap: "Shapes in R graphics"
...
For example, we'll take the `heightweight` data set and add another column that indicates whether the child weighed 100 pounds or more (Figure @fig-scatter-shapes-fill):

```{r}
#| label: fig-scatter-shapes-fill
#| fig-cap: "A variable mapped to shape and another mapped to fill"
...
For more information about recoding a continuous variable to a categorical one, see Recipe @sec-recipe-dataprep-recode-continuous.


## Mapping a Continuous Variable to Color or Size {#sec-recipe-scatter-continuous-scatter}
...
The basic scatter plot in Recipe @sec-recipe-scatter-basic-scatter shows the relationship between the continuous variables `ageYear` and `heightIn`. We can represent a third continuous variable, `weightLb`, by mapping this variable to another aesthetic property, such as `colour` or `size` (Figure @fig-scatter-continuous-color-size:

```{r}
#| label: fig-scatter-continuous-color-size
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A continuous variable mapped to colour (left); Mapped to size (right)"
...
There is another consideration when mapping a variable to `size`, which is that the results can be perceptually misleading. While the largest dots in Figure @fig-scatter-continuous-color-size are about 36 times the size of the smallest ones, they are only supposed to represent about 3.5 times the weight of the smallest dots.
...
Mapping a continuous variable to an aesthetic doesn't prevent us from mapping a categorical variable to other aesthetics. In Figure @fig-scatter-continuous-size-categorical-color, we'll map `weightLb` to `size`, and also map `sex` to `color`. Because there is a fair amount of overplotting (where the points overlap), we'll make the points 50% transparent by setting `alpha = .5`. We'll also use `scale_size_area()` to make the area of the points proportional to the data values (see Recipe @sec-recipe-scatter-balloon), and manually change the color palette:

```{r}
#| label: fig-scatter-continuous-size-categorical-color
#| echo: false
#| fig-cap: "Continuous variable mapped to size and categorical variable mapped to colour"
...
To use different colors from the default, see Recipe @sec-recipe-colors-palette-continuous.

See Recipe @sec-recipe-scatter-balloon for creating a balloon plot.


## Dealing with Overplotting {#sec-recipe-scatter-overplot}
...
With large data sets, the points in a scatter plot may overlap and obscure each other and prevent the viewer from accurately assessing the distribution of the data. This is called *overplotting*. If the amount of overplotting is low, you may be able to alleviate the problem by using smaller points, or by using a different shape (like shape 1, a hollow circle) through which other points can be seen. Figure @fig-scatter-basic-shape-size in Recipe @sec-recipe-scatter-basic-scatter demonstrates both of these solutions.
...
The scatter plot in Figure @fig-scatter-overplot contains about 54,000 points. They are heavily overplotted, making it impossible to get a sense of the relative density of points in different areas of the graph:
...
#| dev: "png"
#| dpi: 300
#| fig-cap: "Overplotting, with about 54,000 points"
#| fig-width: 4
#| fig-height: 4
# We'll use the diamonds data set and create a base plot called `diamonds_sp`
diamonds_sp <- ggplot(diamonds, aes(x = carat, y = price))
...
We can make the points semitransparent using the `alpha` aesthetic, as in Figure @fig-scatter-overplot-alpha. Here, we'll make them 90% transparent and then 99% transparent, by setting `alpha = .1` and `alpha = .01`:
...
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Semitransparent points with alpha=.1 (left); With alpha=.01 (right)"
#| fig-width: 4
#| fig-height: 4
diamonds_sp +
  geom_point(alpha = .1)
...
```{r}
#| label: fig-scatter-overplot-bin2d
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Binning data with `stat_bin2d()` (left); With more bins, manually specified colors, and legend breaks (right)"
diamonds_sp +
  stat_bin2d() +
  theme(
    axis.text = element_text(size = 6),
...
Another alternative is to bin the data into hexagons instead of rectangles, with `stat_binhex()` (Figure @fig-scatter-overplot-binhex). It works just like `stat_bin2d()`. To use `stat_binhex()`, you must first install the hexbin package, with the command `install.packages("hexbin")`:


```{r}
#| label: fig-scatter-overplot-binhex
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Binning data with `stat_binhex()` (left); Cells outside of the range shown in grey (right)"
...
For both of these methods, if you manually specify the range and there is a bin that falls outside that range because it has too many or too few points, that bin will show up as grey rather than the color at the high or low end of the range, as seen in the graph on the right in Figure @fig-scatter-overplot-binhex.

Overplotting can also occur when the data is *discrete* on one or both axes, as shown in Figure @fig-scatter-overplot-jitter. In these cases, you can randomly *jitter* the points with `position_jitter()`. By default the amount of jitter is 40% of the resolution of the data in each direction, but these amounts can be controlled with `width` and `height`:

```{r}
#| label: fig-scatter-overplot-jitter
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Data with a discrete x variable (left); Jittered (middle); Jittered horizontally only (right)"
#| fig-width: 4
#| fig-height: 4
...
Instead of binning the data, it may be useful to display a 2D density estimate. To do this, see Recipe @sec-recipe-distribution-density2d.


## Adding Fitted Regression Model Lines {#sec-recipe-scatter-fitlines}
...
By default, `stat_smooth()` also adds a 95% confidence region for the regression fit. The confidence interval can be changed by modifying the value for `level`, or it can be disabled with `se = FALSE` (Figure @fig-scatter-fit-lm):
...
The default color of the fit line is blue. This can be change by setting `colour`. As with any other line, the attributes `linetype` and `size` can also be set. To emphasize the line, you can make the dots less prominent by changing the `colour` of the points (Figure @fig-scatter-fit-lm, bottom right):
...
```{r}
#| label: fig-scatter-fit-lm
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "An lm fit with the default 95\\% confidence region (top left); A 99\\% confidence region (top right); No confidence region (bottom left); In black with grey points (bottom right)"
#| fig-width: 4
#| fig-height: 4
...
The linear regression line is not the only way of fitting a model to the data -- in fact, it's not even the default. If you add `stat_smooth()` without specifying the method, it will use a LOESS (locally weighted polynomial) curve by default, as shown in Figure @fig-scatter-fit-loess:
...
Although there are many attributes we could examine, for this example we'll just look at the relationship of `V1` (clump thickness) and the `class` of the tumor. Because there is a large degree of overplotting, we'll jitter the points and make them semitransparent (`alpha = 0.4`), hollow (`shape = 21`), and slightly smaller (`size = 1.5`). Then we'll add a fitted logistic regression line (Figure @fig-scatter-fit-logistic) by telling `stat_smooth()` to use the `glm()` function with `family = binomial`:

```{r}
#| label: fig-scatter-fit-logistic
#| fig-cap: "A logistic model"
...
If you want the lines to extrapolate from the data, as shown in the right-hand image of Figure @fig-scatter-fit-group, you must use a model method that allows extrapolation, like `lm()`, and pass `stat_smooth()` the option `fullrange = TRUE`:

```{r}
#| label: fig-scatter-fit-group-2
#| eval: false
hw_sp +
...
```{r}
#| label: fig-scatter-fit-group
#| ref-label: !expr c("fig-scatter-fit-group-1", "fig-scatter-fit-group-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "LOESS fit lines for each group (left); Extrapolated linear fit lines (right)"
#| message: false
```
...
## Adding Fitted Lines from an Existing Model {#sec-recipe-scatter-fitlines-model}
...
We can now plot the data points along with the values predicted from the model (as you'll see in Figure @fig-scatter-fit-model):

```{r}
#| label: fig-scatter-fit-model-1
#| eval: false
...
With the heightweight data set, we'll make a linear model with `lm()` and a LOESS model with `loess()` (Figure @fig-scatter-fit-model):
...
```{r}
#| label: fig-scatter-fit-model
#| ref-label: !expr c("fig-scatter-fit-model-1", "fig-scatter-fit-model-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A quadratic prediction line from an lm object (left); Prediction lines from linear (in red) and LOESS (in blue) models (right)"
#| fig-width: 4
#| fig-height: 4
```
...
To illustrate this, we'll use the `biopsy` data set from the `MASS` package. As we did in Recipe @sec-recipe-scatter-fitlines, we'll use `V1` to predict `class`. Since logistic regressions require numeric values from 0 to 1, we need to convert the factor `class` to 0s and 1s:
...
Finally, we'll make the graph with jittered points and the `fitlogistic` line. We'll make the line in a shade of blue by specifying a color in RGB values, and slightly thicker, with `size = 1` (Figure @fig-scatter-fit-model-logistic):

```{r}
#| label: fig-scatter-fit-model-logistic
#| fig-cap: "A fitted logistic model"
...
## Adding Fitted Lines from Multiple Existing Models {#sec-recipe-scatter-fitlines-model-multi}
...
Use the `predictvals()` function from the previous recipe (@sec-recipe-scatter-fitlines-model) along with functions from the `dplyr` package, including `group_by()` and `do()`.
...
Now that we have the list of model objects, we can run the `predictvals()` as defined in @sec-recipe-scatter-fitlines-model to get the predicted values from each model:
...
Finally, we can plot the data with the predicted values (Figure @fig-scatter-fit-model-multi):
...
```{r}
#| label: fig-scatter-fit-model-multi
#| echo: false
#| fig-cap: "Predictions from two separate lm objects, one for each subset of data (left); With facets (right)"
#| fig-width: 12
...
```{r}
#| label: fig-scatter-fit-model-multi-range
#| fig-cap: "Predictions for each group extend to the full x range of all groups together"
ggplot(heightweight, aes(x = ageYear, y = heightIn, colour = sex)) +
  geom_point() +
  geom_line(data = predvals)
```

As you can see in Figure @fig-scatter-fit-model-multi-range, the line for males now extends as far to the right as the line for females. Keep in mind that extrapolating past the data isn't always appropriate, though; whether or not it's justified will depend on the nature of your data and the assumptions you bring to the table.


## Adding Annotations with Model Coefficients {#sec-recipe-scatter-fitlines-text}
...
To add simple text to a plot, simply add an annotation. In this example, we'll create a linear model and use the `predictvals()` function defined in Recipe @sec-recipe-scatter-fitlines-model to create a prediction line from the model. Then we'll add an annotation:
...
This shows that the *r^2* value is 0.4249. We'll create a graph and manually add the text using `annotate()` (Figure @fig-scatter-fit-model-text):
...
```{r}
#| label: fig-scatter-fit-model-text
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Plain text (left); Math expression (right)"
#| fig-width: 3.5
#| fig-height: 3.5
pred <- predictvals(model, "ageYear", "heightIn")
...
Now that we have the expression string, we can add it to the plot. In this example we'll put the text in the bottom-right corner, by setting `x = Inf` and `y = -Inf` and using horizontal and vertical adjustments so that the text all fits inside the plotting area (Figure @fig-scatter-fit-model-text-auto):

```{r}
#| label: fig-scatter-fit-model-text-auto
#| fig-cap: "Scatter plot with automatically generated expression"
#| fig-width: 4
#| fig-height: 4
hw_sp +
  annotate(
...
The math expression syntax in R can be a bit tricky. See Recipe @sec-recipe-annotate-text-math for more information.


## Adding Marginal Rugs to a Scatter Plot {#sec-recipe-scatter-rug}
...
Use `geom_rug()`. For this example (Figure @fig-scatter-rug), we'll use the `faithful` data set. This data set has two columns with data about the Old Faithful geyser: `eruptions`, which is the length of each eruption, and `waiting`, which is the length of time until the next eruption:

```{r}
#| label: fig-scatter-rug
#| fig-cap: "Marginal rug added to a scatter plot"
#| fig-width: 5
#| fig-height: 5
...
In this particular data set, the marginal rug is not as informative as it could be. The resolution of the `waiting` variable is in whole minutes, and because of this, the rug lines have a lot of overplotting. To reduce overplotting, we can jitter the line positions and make them slightly thinner by specifying size (Figure @fig-scatter-rug-jitter). This helps the viewer see the distribution more clearly:

```{r}
#| label: fig-scatter-rug-jitter
#| fig-cap: "Marginal rug with thinner, jittered lines"
#| fig-width: 5
#| fig-height: 5
ggplot(faithful, aes(x = eruptions, y = waiting)) +
  geom_point() +
...
For more about overplotting, see Recipe @sec-recipe-scatter-overplot.


## Labeling Points in a Scatter Plot {#sec-recipe-scatter-labels}
...
#| ref-label: !expr c("fig-scatter-label-1", "fig-scatter-label-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A scatter plot with manually labeled points (left); With automatically labeled points and a smaller font (right)"
#| fig-width: 4.5
#| fig-height: 4.5
```

As you can see in the center of (Figure @fig-scatter-label, right), you may find yourself with a plot where labels are overlapping. To automatically adjust point labels so that they don't overlap, we can use `geom_text_repel` (Figure @fig-ggrepel-label, left) or `geom_label_repel` (which adds a box around the label, Figure @fig-ggrepel-label, right) from the ggrepel package, which functions similarly to `geom_text`.



```{r}
#| label: fig-ggrepel-label
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A scatter plot labeled with `geom_text_repel` (left); Labeled with `geom_label_repel` (right)"
#| fig-width: 4.5
#| fig-height: 4.5
...
Setting `vjust = 0` will make the baseline of the text on the same level as the point (Figure @fig-scatter-label-vjust, left), and setting `vjust = 1` will make the top of the text level with the point. This usually isn't enough, though -- you can increase or decrease `vjust` to shift the labels higher or lower, or you can add or subtract a bit to or from the y mapping to get the same effect (Figure @fig-scatter-label-vjust, right):

```{r}
#| label: fig-scatter-label-vjust
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A scatter plot with vjust=0 (left); With a little extra added to y (right)"
#| fig-width: 4.5
#| fig-height: 4.5
countries_sp +
...
Besides right- or left-justifying all of your labels, you can also adjust the position of all of the labels at once is to use `position = position_nudge()`. This allows you to specify the amount of vertical or horizontal distance you want to move the labels. As you can see from the figures below (Figure @fig-scatter-label-nudge, this strategy works best when there are fewer labels, or fewer points that can cause overlap with labels. Note that the units you specify with `x = ...` and `y = ...` correspond to the units of the x and y axis.

```{r}
#| label: fig-scatter-label-nudge
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Original scatter plot (left); Scatter plot with labels nudged down and to the right (right)"
#| fig-width: 4.5
#| fig-height: 4.5
...
Now we can make the plot (Figure @fig-scatter-label-select). This time, we'll also expand the *x* range so that the text will fit:

```{r}
#| label: fig-scatter-label-select
#| fig-cap: "Scatter plot with selected labels and expanded x range"
...
Finally, another option is to save the output to a vector format such as PDF or SVG (see Recipes Recipe @sec-recipe-output-vector and Recipe @sec-recipe-output-vector-svg), then edit it in a program like Illustrator or Inkscape.

### See Also

For more on controlling the appearance of the text, see Recipe
@sec-recipe-appearance-text-appearance.

If you want to manually edit a PDF or SVG file, see Recipe
@sec-recipe-output-edit-vector.


## Creating a Balloon Plot {#sec-recipe-scatter-balloon}
...
If we just map `GDP` to `size`, the value of `GDP` gets mapped to the *radius* of the dots (Figure @fig-scatter-balloon, left), which is not what we want; a doubling of value results in a quadrupling of area, and this will distort the interpretation of the data. We instead want to map the value of `GDP` to the *area* of the dots, which we can do this using `scale_size_area()` (Figure @fig-scatter-balloon, right):

```{r}
#| label: fig-scatter-balloon
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Balloon plot with value mapped to radius (left); With value mapped to area (right)"
...
The example here is a scatter plot, but that is not the only way to use balloon plots. It may also be useful to use balloon plots to represent values on a grid, where the x- and y-axes are categorical, as in Figure @fig-scatter-balloon-cat:

```{r}
#| label: fig-scatter-balloon-cat
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Balloon plot with categorical axes and text labels (left); With guide points to help position text (right)"
#| fig-width: 4.5
#| fig-height: 4.5
...
To add labels to the circles, see Recipes Recipe @sec-recipe-scatter-labels and Recipe @sec-recipe-annotate-text.

See Recipe @sec-recipe-scatter-continuous-scatter for ways of mapping variables to other aesthetics in a scatter plot.


## Making a Scatter Plot Matrix {#sec-recipe-scatter-splom}
...
To make the scatter plot matrix (Figure @fig-scatter-splom), we'll use all of the variables except for `Name`, since making a scatter plot matrix using the names of the countries wouldn't make sense and  would produce strange-looking results:

```{r}
#| label: fig-scatter-splom
#| fig-cap: "A scatter plot matrix"
#| fig-width: 7
#| fig-height: 7
...
We'll also throw in one more thing: `panel.smooth` for the lower panels, which makes a scatter plot and adds a LOWESS smoothed line, as shown in Figure @fig-scatter-splom-panels. (LOWESS is slightly different from LOESS, which we saw in Recipe @sec-recipe-scatter-fitlines, but the differences aren't important for this sort of rough exploratory visualization):

```{r}
#| label: fig-scatter-splom-panels
#| fig-cap: "Scatter plot with correlations in the upper triangle, smoothing lines in the lower triangle, and histograms on the diagonal"
#| fig-width: 7
#| fig-height: 7
pairs(
  c2009_num,
...
We'll also use small points in the visualization, so that we can distinguish them a bit better (Figure @fig-scatter-splom-panels2). This is done by setting `pch = "."`:

```{r}
#| label: fig-scatter-splom-panels2
#| fig-cap: "Scatter plot matrix with smaller points and linear fit lines"
#| fig-width: 7
#| fig-height: 7
pairs(
  c2009_num,
...
To create a correlation matrix, see Recipe @sec-recipe-miscgraph-corrmatrix.
