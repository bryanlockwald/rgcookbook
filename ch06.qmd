...
# Summarized Data Distributions {#sec-chapter-distribution}

This chapter explores how to visualize summarized distributions of data.

## Making a Basic Histogram {#sec-recipe-distribution-basic-hist}
...
Use `geom_histogram()` and map a continuous variable to x (Figure @fig-distribution-hist-basic):

```{r}
#| label: fig-distribution-hist-basic
#| fig-cap: "A basic histogram"
#| message: false
...
In addition, the default colors -- a dark fill without an outline -- can make it difficult to see which bar corresponds to which value, so we'll also change the colors, as shown in Figure @fig-distribution-hist-width.

```{r}
#| label: fig-distribution-hist-width
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Histogram with binwidth = 5 and with different colors (left); With 15 bins (right)"
...
Sometimes the appearance of the histogram will be very dependent on the width of the bins and where the boundary points between the bins are. In Figure @fig-distribution-hist-boundary, we'll use a bin width of 8. In the version on the left, we'll use the origin parameter to put boundaries at 31, 39, 47, etc., while in the version on the right, we'll shift it over by 4, putting boundaries at 35, 43, 51, etc.:

```{r}
#| label: fig-distribution-hist-boundary
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Different appearance of histograms with the origin at 31 and 35"
# Save a base plot
...
Frequency polygons provide a better way of visualizing multiple distributions without the bars interfering with each other. See Recipe @sec-recipe-distribution-freqpoly.


## Making Multiple Histograms from Grouped Data {#sec-recipe-distribution-multi-hist}
...
Use `geom_histogram()` and use facets for each group, as shown in Figure @fig-distribution-multi-histogram-facet:

```{r}
#| label: fig-distribution-multi-histogram-facet
#| fig-cap: "Two histograms with facets (left); With different facet labels (right)"
#| message: false
...
Now when we plot our modified data frame, our desired labels appear (Figure @fig-distribution-multi-histogram-facet-labels).

```{r}
#| label: fig-distribution-multi-histogram-facet-labels
#| fig-cap: "Histograms with new facet labels"
#| message: false
...
With facets, the axes have the same *y* scaling in each facet. If your groups have different sizes, it might be hard to compare the *shapes* of the distributions of each one. For example, see what happens when we facet the birth weights by `race` (Figure @fig-distribution-multi-histogram-facet-scalesfree, left):

```{r}
#| label: fig-distribution-multi-histogram-facet-scalesfree-1
#| eval: false
ggplot(birthwt, aes(x = bwt)) +
  geom_histogram(fill = "white", colour = "black") +
  facet_grid(race ~ .)
...
To allow the *y* scales to be resized independently (Figure @fig-distribution-multi-histogram-facet-scalesfree, right), use `scales = "free"`. Note that this will only allow the *y* scales to be free -- the *x* scales will still be fixed because the histograms are aligned with respect to that axis:

```{r}
#| label: fig-distribution-multi-histogram-facet-scalesfree-2
#| eval: false
ggplot(birthwt, aes(x = bwt)) +
  geom_histogram(fill = "white", colour = "black") +
...
```{r}
#| label: fig-distribution-multi-histogram-facet-scalesfree
#| ref-label: !expr c("fig-distribution-multi-histogram-facet-scalesfree-1", "fig-distribution-multi-histogram-facet-scalesfree-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Histograms with the default fixed scales (left); With scales = \"free\" (right)"
#| fig-width: 4
#| fig-height: 4
#| message: false
```

Another approach is to map the grouping variable to `fill`, as shown in Figure @fig-distribution-multi-histogram-fill. The grouping variable must be a factor or a character vector. In the `birthwt` data set, the desired grouping variable, `smoke`, is stored as a number, so we’ll use the `birthwt_mod` data set we created above, in which smoke is a factor:

```{r}
#| label: fig-distribution-multi-histogram-fill
#| fig-cap: "Multiple histograms with different fill colors"
#| message: false
...
## Making a Density Curve {#sec-recipe-distribution-basic-density}
...
Use `geom_density()` and map a continuous variable to x (Figure @fig-distribution-density-basic):

```{r}
#| label: fig-distribution-density-basic-1
#| eval: false
ggplot(faithful, aes(x = waiting)) +
  geom_density()
```

If you don't like the lines along the side and bottom, you can use `geom_line(stat = "density")` (see Figure @fig-distribution-density-basic, right):

```{r}
#| label: fig-distribution-density-basic-2
#| eval: false
...
```{r}
#| label: fig-distribution-density-basic
#| ref-label: !expr c("fig-distribution-density-basic-1", "fig-distribution-density-basic-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A kernel density estimate curve with `geom_density()` (left); With `geom_line()` (right)"
#| fig-width: 4
#| fig-height: 4
```
...
A kernel density curve is an estimate of the population distribution, based on the sample data. The amount of smoothing depends on the *kernel bandwidth*: the larger the bandwidth, the more smoothing there is. The bandwidth can be set with the `adjust` parameter, which has a default value of 1. Figure @fig-distribution-density-adjust shows what happens with a smaller and larger value of `adjust`:

```{r}
#| label: fig-distribution-density-adjust
#| fig-cap: "Density curves with adjust set to .25 (red), default value of 1 (black), and 2 (blue)"
#| fig-width: 4
#| fig-height: 4
ggplot(faithful, aes(x = waiting)) +
  geom_line(stat = "density") +
...
In this example, the *x* range is automatically set so that it contains the data, but this results in the edge of the curve getting clipped. To show more of the curve, set the *x* limits (Figure @fig-distribution-density-width). We'll also add an 80% transparent fill, with `alpha = .2`:



```{r}
#| label: fig-distribution-density-width
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Density curve with wider x limits and a semitransparent fill (left); In two parts, with `geom_density()` and `geom_line()` (right)"
#| fig-width: 4
#| fig-height: 4
ggplot(faithful, aes(x = waiting)) +
...
See Recipe @sec-recipe-distribution-violin for information on violin plots, which are another way of representing density curves and may be more appropriate for comparing multiple distributions.


## Making Multiple Density Curves from Grouped Data {#sec-recipe-distribution-multi-density}
...
Use `geom_density()`, and map the grouping variable to an aesthetic like `colour` or `fill`, as shown in Figure @fig-distribution-multi-density. The grouping variable must be a factor or a character vector. In the `birthwt` data set, the desired grouping variable, `smoke`, is stored as a number, so we have to convert it to a factor first.

```{r}
#| label: fig-distribution-multi-density
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Different line colors for each group (left); Different semitransparent fill colors for each group (right)"
library(MASS) # Load MASS for the birthwt data set
...
Another method for visualizing the distributions is to use facets, as shown in Figure @fig-distribution-multi-density-facet. We can align the facets vertically or horizontally. Here we'll align them vertically so that it's easy to compare the two distributions:

```{r}
#| label: fig-distribution-multi-density-facet-1
#| eval: false
ggplot(birthwt_mod, aes(x = bwt)) +
  geom_density() +
  facet_grid(smoke ~ .)
...
```{r}
#| label: fig-distribution-multi-density-facet-2
#| eval: false
...
@fig-distribution-multi-density-facet, right):

```{r}
#| label: fig-distribution-multi-density-facet-3
#| eval: false
ggplot(birthwt_mod, aes(x = bwt)) +
  geom_density() +
  facet_grid(smoke ~ .)
...
```{r}
#| label: fig-distribution-multi-density-facet
#| ref-label: !expr c("fig-distribution-multi-density-facet-1", "fig-distribution-multi-density-facet-2", "fig-distribution-multi-density-facet-3")
#| echo: false
#| results: "hide"
#| fig-show: "hold"
#| fig-subcap: ["", "", ""]
#| layout-ncol: 3
#| fig-cap: "Density curves with facets (left); With different facet labels (right)"
#| fig-width: 4
#| fig-height: 4
```

If you want to see the histograms along with the density curves, the best option is to use facets, since other methods of visualizing both histograms in a single graph can be difficult to interpret. To do this, map `y = ..density..`, so that the histogram is scaled down to the height of the density curves. In this example, we'll also make the histogram bars a little less prominent by changing the colors (Figure @fig-distribution-multi-density-hist):

```{r}
#| label: fig-distribution-multi-density-hist
#| fig-cap: "Density curves overlaid on histograms"
#| fig-width: 4
#| fig-height: 4
...
## Making a Frequency Polygon {#sec-recipe-distribution-freqpoly}
...
Use geom_`freqpoly()` (Figure @fig-distribution-freqpoly):

```{r}
#| label: fig-distribution-freqpoly-1
#| eval: false
ggplot(faithful, aes(x=waiting)) +
  geom_freqpoly()
```
...
Like with a histogram, you can control the bin width for the frequency polygon (Figure @fig-distribution-freqpoly, right):

```{r}
#| label: fig-distribution-freqpoly-2
#| eval: false
ggplot(faithful, aes(x = waiting)) +
...
```{r}
#| label: fig-distribution-freqpoly
#| ref-label: !expr c("fig-distribution-freqpoly-1", "fig-distribution-freqpoly-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "A frequency polygon (left); With wider bins (right)"
#| fig-width: 4
#| fig-height: 4
#| message: false
```
...
Histograms display the same information, but with bars instead of lines. See Recipe @sec-recipe-distribution-basic-hist.


## Making a Basic Box Plot {#sec-recipe-distribution-basic-boxplot}
...
Use `geom_boxplot()`, mapping a continuous variable to y and a discrete variable to x (Figure @fig-distribution-boxplot-basic):

```{r}
#| label: fig-distribution-boxplot-basic
#| fig-cap: "A box plot"
library(MASS) # Load MASS for the birthwt data set
...
@fig-distribution-boxplot-width-point, left):

```{r}
#| label: fig-distribution-boxplot-width-point-1
#| eval: false
ggplot(birthwt, aes(x = factor(race), y = bwt)) +
  geom_boxplot(width = .5)
```

If there are many outliers and there is overplotting, you can change the size and shape of the outlier points with `outlier.size` and `outlier.shape`. The default size is 2 and the default shape is 16. This will use smaller points, and hollow circles (Figure @fig-distribution-boxplot-width-point, right):

```{r}
#| label: fig-distribution-boxplot-width-point-2
#| eval: false
ggplot(birthwt, aes(x = factor(race), y = bwt)) +
...
```{r}
#| label: fig-distribution-boxplot-width-point
#| ref-label: !expr c("fig-distribution-boxplot-width-point-1", "fig-distribution-boxplot-width-point-2")
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Box plot with narrower boxes (left); With smaller, hollow outlier points (right)"
#| fig-width: 3.5
#| fig-height: 3.5
```

To make a box plot of just a single group, we have to provide some arbitrary value for x; otherwise, ggplot won't know what *x* coordinate to use for the box plot. In this case, we'll set it to 1 and remove the x-axis tick markers and label (Figure @fig-distribution-boxplot-single):

```{r}
#| label: fig-distribution-boxplot-single
#| fig-cap: "Box plot of a single group"
#| fig-width: 3
#| fig-height: 3.5
...
## Adding Notches to a Box Plot {#sec-recipe-distribution-boxplot-notch}
...
@fig-distribution-boxplot-notch):

```{r}
#| label: fig-distribution-boxplot-notch
#| fig-cap: "A notched box plot"
#| message: false
library(MASS) # Load MASS for the birthwt data set
...
## Adding Means to a Box Plot {#sec-recipe-distribution-boxplot-mean}
...
Use `stat_summary()`. The mean is often shown with a diamond, so we'll use shape 23 with a white fill. We'll also make the diamond slightly larger by setting `size = 3` (Figure @fig-distribution-boxplot-mean):

```{r}
#| label: fig-distribution-boxplot-mean
#| fig-cap: "Mean markers on a box plot"
library(MASS) # Load MASS for the birthwt data set
...
## Making a Violin Plot {#sec-recipe-distribution-violin}
...
Use `geom_violin()` (Figure @fig-distribution-violin-basic):

```{r}
#| label: fig-distribution-violin-basic
...
A violin plot is a kernel density estimate, mirrored so that it forms a symmetrical shape. Traditionally, they also have narrow box plots overlaid, with a white dot at the median, as shown in Figure @fig-distribution-violin-boxplot. Additionally, the box plot outliers are not displayed, which we do by setting `outlier.colour = NA`:

```{r}
#| label: fig-distribution-violin-boxplot
#| fig-cap: "A violin plot with box plot overlaid on it"
#| fig-width: 3.5
hw_p +
  geom_violin() +
...
The default range goes from the minimum to maximum data values; the flat ends of the violins are at the extremes of the data. It's possible to keep the tails, by setting `trim = FALSE` (Figure @fig-distribution-violin-tail):

```{r}
#| label: fig-distribution-violin-tail
#| fig-cap: "A violin plot with tails"
#| fig-width: 3.5
hw_p +
  geom_violin(trim = FALSE)
```

By default, the violins are scaled so that the total area of each one is the same (if `trim = TRUE`, then it scales what the area *would be* including the tails). Instead of equal areas, you can use `scale = "count"` to scale the areas proportionally to the number of observations in each group (Figure @fig-distribution-violin-scalecount). In this example, there are slightly fewer females than males, so the female violin becomes slightly narrower than before:

```{r}
#| label: fig-distribution-violin-scalecount
#| fig-cap: "Violin plot with area proportional to number of observations"
#| fig-width: 3.5
...
To change the amount of smoothing, use the adjust parameter, as described in Recipe @sec-recipe-distribution-basic-density. The default value is 1; use larger values for more smoothing and smaller values for less smoothing (Figure @fig-distribution-violin-adjust):

```{r}
#| label: fig-distribution-violin-adjust
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Violin plot with more smoothing (left); With less smoothing (right)"
#| fig-width: 3.5
# More smoothing
hw_p +
...
To create a traditional density curve, see Recipe @sec-recipe-distribution-basic-density.

To use different point shapes, see Recipe @sec-recipe-line-graph-point-appearance.


## Making a Dot Plot {#sec-recipe-distribution-dot-plot}
...
Use `geom_dotplot()`. For this example (Figure @fig-distribution-dotplot-basic), we'll use a subset of the `countries` data set:

```{r}
#| label: fig-distribution-dotplot-basic
#| fig-cap: "A dot plot"
#| message: false
...
This kind of dot plot is sometimes called a *Wilkinson* dot plot. It's different from the Cleveland dot plots shown in Recipe @sec-recipe-bar-graph-dot-plot. In these Wilkinson dot plots, the placement of the bins depends on the data, and the width of each dot corresponds to the maximum width of each bin. The maximum bin size defaults to 1/30 of the range of the data, but it can be changed with binwidth.

By default, `geom_dotplot()` bins the data along the x-axis and stacks on the y-axis. The dots are stacked visually, and due to technical limitations of ggplot2, the resulting graph has y-axis tick marks that aren't meaningful. The y-axis labels can be removed by using `scale_y_continuous()`. In this example, we'll also use `geom_rug()` to show exactly where each data point is (Figure @fig-distribution-dotplot-no-y-rug):

```{r}
#| label: fig-distribution-dotplot-no-y-rug
#| fig-cap: "Dot plot with no y labels, max bin size of .25, and a rug showing each data point"
c2009_p +
...
The dots can also be stacked centered, or centered in such a way that stacks with even and odd quantities stay aligned. This can by done by setting `stackdir = "center"` or `stackdir = "centerwhole"`, as illustrated in Figure @fig-distribution-dotplot-center:

```{r}
#| label: fig-distribution-dotplot-center
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Dot plot with stackdir = \"center\" (left); With stackdir = \"centerwhole\" (right)"
#| fig-width: 3.5
#| fig-height: 3.5
c2009_p +
...
## Making Multiple Dot Plots for Grouped Data {#sec-recipe-distribution-dot-plot-multi}
...
To compare multiple groups, it's possible to stack the dots along the y-axis, and group them along the x-axis, by setting `binaxis = "y"`. For this example, we'll use the heightweight data set (Figure @fig-distribution-dotplot-multi):

```{r}
#| label: fig-distribution-dotplot-multi
#| fig-cap: "Dot plot of multiple groups, binning along the y-axis"
library(gcookbook) # Load gcookbook for the heightweight data set
...
## Making a Density Plot of Two-Dimensional Data {#sec-recipe-distribution-density2d}
...
Use `stat_density2d()`. This makes a 2D kernel density estimate from the data. First we'll plot the density contour along with the data points (Figure @fig-distribution-density2d, left):

```{r}
#| label: fig-distribution-density2d-1
#| eval: false
...
It's also possible to map the *height* of the density curve to the color of the contour lines, by using `..level..` (Figure @fig-distribution-density2d, right):

```{r}
#| label: fig-distribution-density2d-2
#| eval: false
...
```{r}
#| label: fig-distribution-density2d
#| echo: false
#| fig-show: "hold"
#| fig-subcap: ["", ""]
#| layout-ncol: 2
#| fig-cap: "Points and density contour (left); With ..level.. mapped to color (right)"
#| fig-width: 10
faithful_p <- ggplot(faithful, aes(x = eruptions, y = waiting))
...
As with the one-dimensional density estimate, you can control the bandwidth of the estimate. To do this, pass a vector for the *x* and *y* bandwidths to `h`. This argument gets passed on to the function that actually generates the density estimate, `kde2d()`. In this example (Figure @fig-distribution-density2d-bandwidth), we'll use a smaller bandwidth in the *x* and *y* directions, so that the density estimate is more closely fitted (perhaps overfitted) to the data:

```{r}
#| label: fig-distribution-density2d-bandwidth
#| fig-cap: "Density plot with a smaller bandwidth in the x and y directions"
faithful_p +
  stat_density2d(
    aes(fill = ..density..),
    geom = "raster",
